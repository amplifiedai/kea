...
- **Phase 2 (active):** MCP probe on a handled-absent resumptive shape (`handle Log.log(1)` with `Trace` clause) rejects with `E0001`, which is expected under well-typedness boundaries (the clause does not handle `Log`); composition claims remain intentionally precondition-gated to well-typed clauses.
