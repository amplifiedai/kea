...
- **Phase 2 (active):** MCP probe on a handled-absent resumptive shape (`handle Log.log(1)` with `Trace` clause) currently rejects with `E0001` (`declared [Log] too weak; body requires [IO]`), so this composition surface is tracked as precondition-gated (well-typed-clause boundary) rather than unconditional runtime alignment.
