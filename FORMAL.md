...
- **Phase 1 (active):** Strengthened the hook-free principal boundary on the no-unify fragment in `Kea/Typing.lean` (`inferExprUnify_complete_no_unify_branches`, `inferExprUnify_state_preserved_no_unify_branches`, `inferExprUnify_ok_iff_inferExpr_no_unify_branches`, `PrincipalTypingNoUnifySlice`) so successful `inferExprUnify` runs are equivalent to `inferExpr` with unchanged `UnifyState`.
