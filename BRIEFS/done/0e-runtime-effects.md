...
- 2026-02-28 00:18: Fixed TCO through handler-cell paths for tail recursion lowered via branch-join blocks. Codegen tail-call detection now recognizes `self-call -> jump(join) -> return` shapes (including Nop-only join blocks) and keeps handler-cell forwarding strict by requiring hidden effect-cell suffix args to be forwarded unchanged. Added codegen regressions for join-return detection (`collect_pass_stats_detects_tail_self_call_through_join_return`, `collect_pass_stats_detects_handler_cell_tail_self_call_through_join_return`) plus a negative guard (`collect_pass_stats_rejects_tail_self_call_when_handler_cell_arg_is_rewritten`) and compile-path coverage (`cranelift_backend_compiles_handler_cell_tail_self_call_through_join_return`). Added execute-path 1M regression `compile_and_execute_state_tail_handler_count_to_one_million_exit_code` and stats regression `compile_state_tail_handler_count_to_marks_tail_self_call_in_stats` to lock real source pipeline behavior.
- 2026-02-28 00:22: Raised runtime benchmark workload to the intended 1,000,000-step state recursion path (`STATE_COUNT_BENCH_N = 1_000_000`) and re-ran Stage-B regression export. Updated `state_count_to_1_m_manual` baseline threshold to the new workload median so regression gating remains apples-to-apples. Current medians from `mise run bench:regression`: `state_count_to_1_m = 572.4µs`, `state_count_to_1_m_manual = 846.9µs` (run green, failures: 0).
- 2026-02-28 00:25: Completed 0e DoD audit checks for remaining verification-only items. Non-tail resumptive behavior remains explicitly guarded by execute-path test `compile_and_reject_non_tail_resumptive_clause_body_with_effects`, which asserts a clear compiled-path diagnostic (`must be tail-resumptive (\`resume ...\`) for compiled lowering`) rather than crash/fallback.
- 2026-02-28 00:25: Added missing benchmark comparators for open performance gates in `kea-bench`: `fail_propagation_depth_n_manual_result` (hand-written Result propagation baseline for depth 8/32/128) and `io_stdout_throughput` / `io_stdout_throughput_libc_write` (direct capability IO path vs direct libc write path). Added baseline threshold rows in `benchmarks/baselines/microbench-thresholds.csv` and reran Stage-B regression (green). Latest medians: Fail path `298.2µs / 1.01ms / 4.917ms` vs manual Result `542µs / 2.06ms / 9.11ms` (all <1.5x gate), IO path `97.68µs` vs libc write `266.8µs` (within 2x gate).
- 2026-02-28 00:27: Final DoD gate run complete: `mise run check-full` passed (workspace clippy + tests + doctests all green) after benchmark gate verification (`mise run bench:regression`, failures: 0). 0e core, performance gates, and infrastructure gates are now satisfied; brief is done and ready to move to `done/`.
