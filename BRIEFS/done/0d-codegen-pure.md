...
- 2026-02-27 12:09: Closed nested payload-constructor pattern lowering on the compiled path (`Just(Just(n))` and similar): HIR constructor-pattern lowering now recursively collects nested payload checks/binds with chained `SumPayloadAccess` paths, so nested constructor arms no longer fall back to `Raw`/unsupported MIR. MIR now propagates sum-type metadata through `SumPayloadLoad` results and rewrites nested payload tag comparisons to `SumTagLoad`-based checks. Added HIR regression `lower_function_payload_constructor_nested_case_binds_nested_payload_slot`, MIR regression `lower_hir_module_rewrites_sum_payload_eq_to_tag_compare`, and source-level CLI regression `compile_and_execute_nested_payload_constructor_case_exit_code` (plus direct probe `type Maybe a = Just(a) | Nothing; case Just(Just(3)) ...` now exits `3` instead of failing with `non-unit function returned without value`).
- 2026-02-27 12:13: Closed nested OR-payload constructor lowering on compiled path (`Wrap(A(n) | B(n))` and similar): literal-case lowering now expands OR alternatives at case-arm level before constructor lowering, so distributed nested constructor alternatives lower as independent concrete arms instead of failing the strict shared-payload-check merge gate. Added HIR regression `lower_function_payload_constructor_nested_or_case_stays_lowered` and source-level CLI regression `compile_and_execute_nested_or_payload_constructor_case_exit_code`; adversarial probe (`p1+p2+p3+p4`) now executes compiled path end-to-end with exit code `49` instead of `unsupported MIR operation in p3`.
- 2026-02-27 12:27: Closed struct+enum integration gap where sum payload annotations referencing records (`type Wrap = W(User) | N`) failed in CLI registration and later fell off compiled path for record field access from payload binds. Registration pipeline now pre-registers aliases/opaques/records before batch sum registration so variant fields can resolve record names regardless of declaration order, and HIR variant metadata now preserves nominal record/sum field types (not only primitive builtins) for constructor payload binds/checks. Added CLI regression `compile_and_execute_sum_payload_record_type_exit_code` (`W(u) -> u.age + 1`) and kept full `kea` binary suite green.
- 2026-02-27 12:31: Closed constructor-payload record-pattern lowering on compiled path (`W(User { age: n })` and nested record-field accesses inside payload patterns). Payload access paths now support both sum-payload and record-field steps, allowing constructor pattern lowering to recursively destructure record fields in payloads rather than falling back to raw/unsupported MIR. Added HIR regression `lower_function_payload_constructor_record_payload_pattern_binds_field` and CLI regression `compile_and_execute_sum_payload_record_pattern_exit_code`; `case W(User { age: 7 })` with payload record destructuring now compiles and executes end-to-end.
...
- 2026-02-27 13:26: Post-scoped-RC benchmark re-check remains green: Stage B micro/program regression tasks both pass with 0 failures; worst micro regression remains bounded (`lower_hir_to_mir(64)` +10.95% vs +30% gate) while whole-program means stay improved (`alloc_chain -52.21%`, `case_chain -44.48%`, `fib -42.80%` vs configured baselines).
- 2026-02-27 14:01: CALL-SYNTAX namespace unification landed on implementation path: parser now accepts `.` as the only namespace/member accessor for postfix and handler clause heads (`List.map`, `xs.Show.show`, `Log.log(...)`), while retaining `ColonColon` token support only for non-access syntax (list-cons patterns). HIR/infer/MIR/codegen/CLI were migrated to dot-qualified symbol keys (`Fail.fail`, `IO.stdout`, `Kea.List.map`) so qualified resolution, effect-op recognition, external-call classification, and trait-qualified runtime lifting stay aligned with parser output. Added parser regressions that `List::map` namespace usage is rejected and updated existing qualified-call/handler tests accordingly; package tests (`kea-syntax`, `kea-hir`, `kea-infer`, `kea-mir`, `kea-codegen`, `kea` bin) plus `mise run check` all pass.
- 2026-02-27 14:16: Benchmark gate calibration moved from local-only to CI-history-backed values: consumed 12 recent GitHub Stage B runs, recalibrated micro/program baseline medians to CI medians, and promoted low-variance parser classes into stable blocking Stage B (`lex_parse_numeric_module` 128/512, `lex_parse_string_transform_module` 64/256). Local verification passes with zero failures across all four regression lanes (`bench:regression`, `bench:regression:stable`, `bench:programs:regression`, `bench:programs:regression:stable`).
- 2026-02-27 14:18: Tightened Fail/catch compiled-path coverage with new source-level CLI regressions for higher-order catch contracts: `catch f()` now has explicit end-to-end tests for both Err and Ok paths when `f` is a parameter typed `fn() -[Fail Int]> Int`, plus rejection coverage for unnecessary catch (`catch 42`) with expected diagnostic. `./scripts/cargo-agent.sh test -p kea --bin kea` and `mise run check` both pass.
- 2026-02-27 14:20: Added an explicit JIT-vs-AOT exit-code parity corpus gate in `kea` CLI tests (`compile_jit_and_aot_exit_code_parity_corpus`) spanning pure arithmetic, record field access, sum pattern matching, higher-order lambda calls, and Fail/catch Result lowering. This locks cross-mode behavioral parity on representative 0d paths and guards against backend drift as runtime lowering expands. `./scripts/cargo-agent.sh test -p kea --bin kea` and `mise run check` pass.
- 2026-02-27 14:23: Expanded direct-expression scrutinee coverage for compiled record destructuring paths: added source-level CLI regressions for named record and anonymous record direct scrutinees (`case User { ... }`, `case #{ ... }`) to guard against recurrence of non-dominating-value / fallback lowering issues outside let-bound scrutinee shapes. Full `kea` bin suite (`83` tests) and `mise run check` pass.
- 2026-02-27 14:40: Closed the remaining 0d punch-list items that do not depend on `kea-eval`: added CLI stdout regression `kea_run_prints_hello_world_stdout` (`kea run` must emit `hello world`), added tail-recursive stress regression `compile_and_execute_tail_recursive_factorial_mod_exit_code` (`fact_mod(100000, 1)`), and added allocation-churn smoke for runtime stability (`compile_and_execute_refcount_allocation_churn_exit_code`, `kea_run_allocation_churn_smoke_does_not_crash`).
- 2026-02-27 14:40: Added a bounded refcount smoke gate on compiled output via codegen pass stats (`compile_emits_release_ops_for_allocation_churn_program`), asserting release activity is present and tracks allocation volume on heap-churn programs.
