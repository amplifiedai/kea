...
- **[Benchmark infrastructure](in-progress/benchmark-infrastructure.md)** — Active. 0d deliverable is now executing: `divan` harness is in-tree (`kea-bench` + `AllocProfiler`) with initial baselines collected; next is stable benchmark artifacts and CI Stage A (publish metrics, no fail gates).
