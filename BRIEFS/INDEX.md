...
- **[Memory model](in-progress/0f-memory-model.md)** — Active. 0f Step 6 now includes numeric literal infrastructure, `Int` wrapping arithmetic + bit-count methods, fixed-width constant-expression overflow diagnostics, and mixed-width integer widening rules in `kea-infer`. Next: fixed-width execute-path lowering in MIR/codegen plus narrowing-explicit end-to-end coverage.
