...
- **[Memory model](in-progress/0f-memory-model.md)** — Active. 0f Step 6 literal infrastructure is now end-to-end for prefixed/underscored numeric forms (lexer + parser + compiled execute-path regressions). Next: fixed-width integer overflow/conversion semantics and wrapping arithmetic surface.
