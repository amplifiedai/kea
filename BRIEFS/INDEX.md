...
- **[Memory model](in-progress/0f-memory-model.md)** — Active. 0f Step 6 now includes numeric literal infrastructure, `Int` wrapping arithmetic + bit-count methods, fixed-width constant-expression overflow diagnostics, mixed-width integer widening rules, and width-accurate codegen lowering for fixed-width numeric signatures (including signedness-aware `IntN/UIntN` JIT `main` decoding and local-call argument coercion against callee signatures). Next: explicit narrowing-conversion surface (`try_from`/checked casts) plus conversion-path overflow diagnostics end-to-end.
