...
- **[Memory model](in-progress/0f-memory-model.md)** — Active. 0f Step 6 now includes end-to-end numeric literal infrastructure plus `Int` wrapping arithmetic methods (`wrapping_add/sub/mul`) through infer→MIR→codegen with execute-path tests. Next: integer bit-count methods and fixed-width conversion/overflow diagnostics.
