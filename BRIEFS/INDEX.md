...
- **[Benchmark infrastructure](in-progress/benchmark-infrastructure.md)** — Active. `divan` harness is in-tree (`kea-bench` + `AllocProfiler`) with initial baselines collected; stable artifacts are exported (`raw/csv/json/meta`) and CI Stage A is live (run + publish artifacts, no fail gates yet).
