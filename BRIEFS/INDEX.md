...
- **[Codegen — pure subset](in-progress/0d-codegen-pure.md)** — Active. Step 0 structural function-effect embedding is complete; Step 1 (`kea-hir`) and Step 2 (`kea-mir`) now include real pure-path lowering for literals/vars/binary/call/let/block; Step 3 backend interface + Cranelift stub are in-tree and tested.
