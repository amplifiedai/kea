...
- **[Memory model](in-progress/0f-memory-model.md)** — Active. 0f Step 6 now includes end-to-end numeric literal infrastructure, `Int` wrapping arithmetic (`wrapping_add/sub/mul`), bit-count methods (`popcount`, `leading_zeros`, `trailing_zeros`), and fixed-width constant-expression overflow diagnostics in `kea-infer`. Next: mixed-width arithmetic coercion rules plus execute-path coverage (widening implicit, narrowing explicit).
