...
- **[Memory model](in-progress/0f-memory-model.md)** — Active. 0f Step 6 now includes end-to-end numeric literal infrastructure, `Int` wrapping arithmetic (`wrapping_add/sub/mul`), and bit-count methods (`popcount`, `leading_zeros`, `trailing_zeros`) through infer→MIR→codegen with execute-path tests. Next: fixed-width conversion and overflow diagnostics.
