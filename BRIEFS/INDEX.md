...
- **[Memory model](in-progress/0f-memory-model.md)** — Active. 0f Step 6 now includes numeric literal infrastructure, `Int` wrapping arithmetic + bit-count methods, fixed-width constant-expression overflow diagnostics, mixed-width integer widening rules, and bootstrap execute-path coverage for typed mixed-width arithmetic programs (including integer-typed `main` JIT dispatch). Next: width-accurate fixed-width codegen lowering and explicit narrowing-conversion semantics end-to-end.
