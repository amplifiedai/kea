...
- **Next:** implement Step 3 runner path (`kea test`): lower top-level `test` declarations through HIR/MIR with implicit Fail handling, remove postfix `testing` blocks (Rill-ism), and execute `.kea` test suites directly without Rust behavioral harnesses.
