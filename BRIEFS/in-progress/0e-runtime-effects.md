...
- **Next:** keep the 0e runtime lane on evidence-dispatch depth: tighten multi-operation handler coverage and promote remaining fixture-only stdlib suites (`io`, `net`, `list`) into the enforced `kea test` corpus as corresponding runtime/call-lowering gaps close.
