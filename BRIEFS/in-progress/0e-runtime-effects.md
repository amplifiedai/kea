...
- **Next:** continue 0e runtime-effects critical path by replacing shape-based cross-boundary fallback with explicit evidence-dispatch semantics and extending handler execution coverage (nesting/scoping + `then`) while keeping `kea test` aligned as effect features land.
