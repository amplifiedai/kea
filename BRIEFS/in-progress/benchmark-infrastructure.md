...
- 2026-02-27 14:52: Added update-fusion optimization-contract coverage for 0d: new no-arg microbench pair in `kea-bench` (`lower_record_update_single_to_mir`, `lower_record_update_chain_to_mir`), new contract checker (`scripts/bench-update-fusion-contract.sh`), and `mise run bench:update-fusion:contract` task. Also fixed `scripts/bench-export.sh` CSV extraction to include no-arg benchmarks so contract rows are emitted and checked correctly.
- 2026-02-27 23:38: Added 0e effect-heavy microbench rows in `kea-bench` + Stage-B baselines: `state_count_to_1_m`, `state_count_to_1_m_manual`, and `fail_propagation_depth_n` (`8/32/128`) now run on the compiled JIT path and are tracked in `microbench-thresholds.csv` (non-blocking lane). Seed medians from `mise run bench:regression`: `691.1µs` (state), `514.0µs` (manual), `293.4µs/1.014ms/4.838ms` (fail depth). `state_count_to_1_m` is temporarily capped at `100_000` iterations pending the known handler recursion stack-overflow issue at `1_000_000` (tracked in 0e brief).
