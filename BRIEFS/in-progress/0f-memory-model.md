...
- **Next:** Use `reuse_token_candidate_count` hotspots to drive the first explicit MIR `ReuseToken` op implementation for loop-carried/non-tail pairings (lower to reuse fast-path + alloc fallback in codegen).
