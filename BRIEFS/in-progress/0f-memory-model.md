...
- **Next:** continue Step 6 by landing overflow + conversion semantics for fixed-width integer types (debug overflow diagnostics for `Int8..Int64`/`UInt8..UInt64`, then wrapping arithmetic methods) with execute-path and infer diagnostics coverage.
