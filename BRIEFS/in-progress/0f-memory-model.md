...
- 2026-02-28 00:50: Wired Step 6 fixed-width execute-path bootstrap coverage: codegen now accepts `Type::IntN`/`Type::FloatN` in scalar lowering (`clif_type`) via the existing scalar ABI, and CLI execute-path regressions now cover mixed-width typed arithmetic programs (`Int8 + Int16`, `UInt8 + UInt16`, `Int8 + UInt8`). To satisfy the current JIT `main` contract (`Int | Unit` return), signed-only and unsigned-only cases are widened to `Int` at the expression boundary (`+ 0`) in the regression sources. Verified with `PKG=kea-codegen mise run test-pkg`, `PKG=kea-infer mise run test-pkg`, `PKG=kea mise run test-pkg`, and `mise run check`.
- **Next:** continue Step 6 by replacing the bootstrap scalar-ABI fallback with width-accurate codegen lowering for `Int8..Int64`/`UInt8..UInt64`, then add explicit narrowing-conversion surface and overflow diagnostics end-to-end.
