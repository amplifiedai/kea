...
- 2026-03-01 23:53: Extended Step 3e benchmark telemetry in `reuse_metrics` with a constructor-context TRMC source kernel (`trmc_chain_weighted`, `Node(n + n, build(n - 1))`) to prove the generalized field-expression rewrite is observed at source level, not only MIR unit tests. Current metrics now report `trmc_candidate_count=0` for both `trmc_chain` and `trmc_chain_weighted` with totals also at zero. Verified with `cargo test -p kea-bench --benches --no-run`, `./scripts/cargo-agent.sh run -p kea-bench --bin reuse_metrics -- /tmp/reuse.metrics.trmc.json`, and `mise run check`.
- 2026-03-01 23:55: Broadened Step 3e TRMC matching for practical arities: `rewrite_trmc_descending_sum_chain` now accepts recursive functions with passthrough extra parameters (arity > 1) as long as non-recursive parameters are forwarded unchanged across the self-call. Added MIR regression `rewrite_trmc_descending_sum_chain_supports_passthrough_extra_params` and kept existing TRMC call-elision regressions green. Verified with `PKG=kea-mir mise run test-pkg` and `mise run check`.
- 2026-03-02 00:02: Added a Step 3e safety guard regression for multi-arg recursion: TRMC now has explicit negative coverage `rewrite_trmc_descending_sum_chain_rejects_rewritten_extra_params`, proving the rewrite does **not** fire when extra parameters are transformed (non-passthrough) before the recursive call. This locks in conservative correctness while keeping the widened positive path. Verified with `PKG=kea-mir mise run test-pkg` and `mise run check`.
- 2026-03-02 00:05: Extended Step 3e regression gating in CI floor checks: `bench-reuse-floor-check.sh` now supports optional `max_trmc_candidates` thresholds per scope, and both reuse-floor baseline CSVs now pin `trmc_chain` + `trmc_chain_weighted` at `0` candidates (with totals also capped at `0`). This makes TRMC rewrite regressions fail the same Stage B floor lane used for reuse-token floors. Verified with `./scripts/bench-reuse-floor-check.sh /tmp/reuse.metrics.trmc.json benchmarks/baselines/reuse-token-floors.csv /tmp/reuse-floor-check-trmc` and `mise run check`.
