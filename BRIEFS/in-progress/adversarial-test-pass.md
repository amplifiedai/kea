...
- **Next:** Continue 0d/0e adversarial expansion with additional closure/handler and nested-handler stress cases; then decide whether to implement or explicitly brief both pinned lowering gaps (handler-then + outer-catch non-Unit lowering, and 50-parameter stack overflow).
