...
- 2026-03-01 12:57: Closed a 0d arithmetic hard-failure found during adversarial probing: `/ 0` and `% 0` literals previously caused process abort at runtime (SIGFPE). Added literal-denominator guard diagnostics in type checking (`division by zero is not allowed` / `modulo by zero is not allowed`) and landed CLI regressions for both forms. Re-verified with `mise run check`, focused `cargo test -p kea` runs for both cases, and `PKG=kea mise run test-pkg` (292/292). Dynamic zero divisors (`x / y` where `y` is runtime zero) remain a separate runtime semantics decision.
- 2026-03-01 18:01: Landed additional 0d/0e + pathological/wicked stress tranche in CLI integration tests: `case` expression in a handler `then` clause, 20-effect-row compile stress (`compile_project`), 20-level nested `Option` type annotation execution path, 100 top-level function chain stress, and 50-expression string interpolation validated end-to-end via AOT stdout capture. Re-verified with `mise run check` and `PKG=kea mise run test-pkg` (301/301). During this, attempted 100-function mutual recursion uncovered a current limitation (undefined forward function references in type inference), so the landed stress case is a 100-function top-level chain and mutual recursion remains an explicit follow-up capability gap.
- 2026-03-01 18:09: Landed wicked-problems follow-up tranche in CLI integration tests: recursive handler installation depth (25 nested installs), fail-after-resume shape locked as current AOT lowering rejection (`Fail-only lowering invariant violated`), and explicit current-gap assertion for pure/effectful mutual recursion (`undefined variable \`eff\``). Added additional state-handler adversarial assertion that nested `State.put` inside a clause is currently rejected by tail-resumptive compiled-lowering rules (clear diagnostic, no panic). Re-verified with `mise run check`, targeted `cargo test -p kea` for each new test, and `PKG=kea mise run test-pkg` (305/305).
