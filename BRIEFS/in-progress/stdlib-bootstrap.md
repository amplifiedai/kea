...
- **Next:** close Tier-1 handler helper deltas: (1) generic effectful callback return correctness (`fn() -[Effect]> T` through handled wrappers), (2) compiled-lowering support for side-effecting arity-1 clauses (needed for `with_stdout_logger`), and (3) post-handle state extraction path for `with_state(...)->(T, S)` / `with_collected_logs(...)->(T, List String)`.
