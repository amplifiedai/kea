...
- 2026-02-28 07:43: Fixed the real `Option` payload-path runtime bug at the MIR root (not stdlib workarounds): sum-tag metadata now propagates for `Type::Option`/`Type::Result` function parameters and call results, so case-tag rewrites lower through `SumTagLoad` instead of raw pointer-vs-tag integer compares. This resolves `Option.unwrap_or(Some(...), fallback)` taking the fallback path and the stdlib corpus `option_tests.kea` SIGILL.
- 2026-02-28 07:43: Restored `Result.from_option` as direct pattern matching (`case opt`), with no `catch`/`unwrap` workaround path. Added stdlib behavior coverage for `from_option(Some(...)) -> Ok(...)` and `from_option(None, err) -> Err(err)`, plus a CLI regression `compile_and_execute_option_payload_case_on_function_param_exit_code`. Validation: `PKG=kea-mir mise run test-pkg`, `PKG=kea mise run test-pkg`, `mise run check` all pass.
- 2026-02-28 07:59: Removed fake `Show.show` placeholder (`""`) and replaced it with a real Int decimal conversion implementation (`show_non_negative` + digit mapping) in `stdlib/show.kea`. Updated stdlib case coverage to validate shape via `Text.length(Show.show(...))` because direct string equality in `kea test` currently triggers a runtime SIGILL path.
- 2026-02-28 07:59: Attempted to promote `Ord.compare` to return `Ordering`; blocked by current trait registration limits (`unknown return type in trait method`) when trait methods reference imported module types. Reverted `Ord.compare` to Int with explicit `TODO(0f)` marker in `stdlib/ord.kea` so stdlib remains green while preserving the intended direction.
- 2026-02-28 07:59: Attempted broad Prelude re-exports; blocked by current import flattening collisions (`DuplicateDefinition("map")`) when multiple modules export same helper names (`Option.map`, `Result.map`, `List.map`). Reverted to smoke prelude and logged this as module resolver debt rather than shipping brittle workarounds.
- 2026-02-28 08:01: Attempted to genericize stdlib List (`type List a = Nil | Cons(a, List a)`) and add polymorphic list tests. Blocked by current typechecker/exhaustiveness behavior: parametric `List a` is treated as builtin list shape (`[]`, `[_, .._]`), making `Nil/Cons` arms unreachable and emitting non-exhaustive errors. Reverted the attempted genericization and logged this as a compiler blocker (List genericity requires removing this Rill-era builtin-list special casing).
...
- 2026-02-28 23:27: Picked up `STDLIB-BOOTSTRAP-NOTE` follow-up and landed callback/effect plumbing required for Tier-1 handler-style APIs: MIR now threads dispatch cells through first-class effectful callbacks (named + lambda wrappers), and codegen-path regressions now pass for `compile_and_execute_effectful_named_callback_parameter_exit_code` and `compile_and_execute_effectful_with_callback_lambda_exit_code`.
- 2026-02-28 23:27: Fixed effect-contract constraint detection for row-tail vars in function-typed parameters (`fn() -[Reader R, e]> T` now counts as constraining `e`), unblocking `-[e]>` declarations that rely on row-tail propagation.
- 2026-02-28 23:27: Added Tier-1 stdlib effect modules `stdlib/state.kea`, `stdlib/log.kea`, and `stdlib/reader.kea` (effect declarations + docs), with real-stdlib execute coverage for each module (`compile_and_execute_real_stdlib_{state,log,reader}_effect_module_exit_code`).
- 2026-02-28 23:27: Probed `Reader.with_reader` as a fully generic helper and found a remaining backend/runtime delta: effectful callback wrappers with generic return `T` currently produce incorrect runtime values (repro: generic `with_reader(...)->T` returns wrong `Int` on execute path). Per brief rule (“no wrong placeholders”), `with_reader` helper is intentionally left out for now; effect declaration remains shipped and tested.
- 2026-03-01 00:27: Closed the Tier-1 handler helper deltas from `STDLIB-BOOTSTRAP-NOTE`: generic callback returns through handled wrappers now execute correctly, side-effecting arity-1 `Log.*` handler clauses lower via callback invocation, and post-handle `State` reads in `then` clauses are wired for `with_state` extraction paths.
- 2026-03-01 00:27: Fixed handler completeness checking to use declared effect operations only (not all module exports), eliminating false diagnostics like `handler for \`State\` is missing clause(s): with_state` when effect modules define helper functions.
- 2026-03-01 00:27: Added bootstrap tuple pipeline needed by `with_state(...)->(T, S)` execute paths: tuple values now lower as synthetic tuple-record layouts in MIR/codegen, parser accepts numeric tuple indices (`pair.0`), and type inference resolves tuple numeric field access. Added execute regression `compile_and_execute_real_stdlib_state_with_state_helper_exit_code`.
- 2026-03-01 00:27: Validation checkpoint: `mise run check`, `PKG=kea-infer mise run test-pkg` (619/619), and `PKG=kea mise run test-pkg` (210/210) all pass.
- 2026-03-01 03:30: Expanded Tier-1 helper surface with docs + execute-path coverage: `Reader.asks`, `IO.{print,println,eprint,eprintln}`, `Net.send_and_recv`, `Clock.elapsed_since`, and `Rand.int_between` (real-stdlib execute regressions added in `crates/kea/src/main_tests.rs`).
- 2026-03-01 03:30: Extended MIR handler callback lowering so `Log` single-arg handler callbacks can dispatch additional effects by threading callback-specific dispatch cells at invoke sites; re-validated callback regressions (`compile_and_execute_effectful_{named_callback_parameter,with_callback_lambda,generic_callback_return}_exit_code`).
- 2026-03-01 03:30: Probed `Log.with_collected_logs` end-to-end and found a remaining runtime blocker: `State.with_state` over heap-managed payloads (e.g. `List String`) currently segfaults on execute path (`EXIT:139`) due state-cell ownership/RC gaps. Per brief rule (“no wrong placeholders”), `with_collected_logs` is intentionally left out for now (helper + execute regression removed), while stable Tier-1 helpers remain shipped and green.
- **Next:** close heap-managed `State` cell ownership semantics in MIR/codegen (safe load/store/move-out for non-immediate payloads), then re-enable `Log.with_collected_logs` with real execute-path assertions on collected message content/order.
