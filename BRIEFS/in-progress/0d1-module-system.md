...
- **Next:** close the remaining runtime/codegen gap for module-struct UMS call execution (current compile-time resolution works, but direct execution of that new path still needs backend alignment) and then upgrade the new compile-only regression to an execute-path regression.
