...
- 2026-02-26: Extended CLI regression coverage with bool-`case` execution path and fixed temp artifact naming collisions in run execution helpers (parallel-test safe unique paths).
- 2026-02-26: HIR `case` canonicalization extended conservatively: bool arms and single literal (`Int`/`Bool`) + wildcard arms lower to `if`; multi-literal non-bool case stays on the explicit non-lowered path until dedicated MIR case lowering lands.
