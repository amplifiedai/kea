...
- 2026-02-26: Targeted MCP probe sweep completed for polymorphic Fail-lowering shapes: Fail-present residual preservation and higher-order case align, but Fail-absent `catch` path diverges (inferred `IO`, and residual `Log` can be dropped), contradicting no-op-if-absent runtime expectations; logged in `formal/mcp-log.md`.
- **Next:** hold no-op-if-absent claims as spec-only and wait for implementation-side fix on Fail-absent `catch` effect propagation before declaring runtime alignment.
