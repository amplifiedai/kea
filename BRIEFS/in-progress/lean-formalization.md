...
- 2026-02-26: MCP probe on handled-absent resumptive shape (`handle Log.log(1)` with `Trace` clause) currently rejects with `E0001` (`declared [Log] too weak; body requires [IO]`), so composition claims are kept precondition-gated to well-typed clauses (logged as precondition gap, not theorem divergence).
