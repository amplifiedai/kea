...
- 2026-02-26: MCP sanity re-probe found a new higher-order divergence: `catch` over parameter `f: fn() -[Log, Fail String]> Int` is rejected as unnecessary (`E0012`), while first-order typed Fail rows still align; logged in `formal/mcp-log.md`.
- **Next:** hold higher-order runtime-alignment claims for admissible catch paths until this regression is fixed; continue only on first-order-aligned theorem surfaces.
