...
- **Next:** connect the new `ResumeUse` scaffold to concrete handler typing premises (especially branch/loop legality constraints), and extend normalized-composition theorems to richer nested-handler interaction cases while tracking the implementation overlap-normalization divergence until dedup lands.
