...
- 2026-02-27 12:45: Strengthened the M4 principal boundary on the hook-free no-unify fragment in `Kea/Typing.lean` by adding completeness/state-preservation/equivalence packaging (`inferExprUnify_complete_no_unify_branches`, `inferExprUnify_state_preserved_no_unify_branches`, `inferExprUnify_ok_iff_inferExpr_no_unify_branches`, `PrincipalTypingNoUnifySlice`), then updated `FORMAL.md` + `formal/ROADMAP.md`; `cd formal && lake build` passes.
