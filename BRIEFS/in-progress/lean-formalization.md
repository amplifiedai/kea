...
- **Next:** extend Phase-2 formalization from removal-only to full handler row composition (removal + handler-body effect addition under explicit disjointness/normalization preconditions), and use the new divergence log to either tighten model preconditions or drive implementation normalization fixes.
