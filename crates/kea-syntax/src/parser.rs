...
impl Parser {
    ...
    fn df_verb(&mut self) -> Option<Expr> {
        ...
        let verb_name = match &name_tok.kind {
            TokenKind::Ident(s) => s.clone(),
            _ => unreachable!(),
        };
        let kind = verb_kind(&verb_name).unwrap();
        let verb = Spanned::new(kind, name_tok.span);
        self.expect(&TokenKind::LParen, "expected '(' after verb name")?;
        self.skip_newlines();

        let args = match kind {
            DfVerbKind::Filter => {
                let expr = self.column_expr()?;
                DfVerbArgs::Filter(expr)
            }
            DfVerbKind::Select => {
                let atoms = self.atom_list()?;
                DfVerbArgs::Select(atoms)
            }
            DfVerbKind::Mutate => {
                let named = self.named_col_expr_list()?;
                DfVerbArgs::Mutate(named)
            }
            DfVerbKind::Update => {
                let atom = self.expect_atom("expected column atom in update")?;
                self.expect(&TokenKind::Comma, "expected ',' after column in update")?;
                self.skip_newlines();
                let expr = self.column_expr()?;
                DfVerbArgs::Update(atom, expr)
            }
            DfVerbKind::GroupBy => {
                let atoms = self.atom_list()?;
                DfVerbArgs::GroupBy(atoms)
            }
            DfVerbKind::Summarize => {
                let named = self.named_col_expr_list()?;
                DfVerbArgs::Summarize(named)
            }
            DfVerbKind::Sort => {
                // Parse atoms, stopping before keyword args (ident followed by :)
                ...
                if !self.check(&TokenKind::RParen) {
                    loop {
                        self.skip_newlines();
                        // If next token is an ident (not :atom), it's a keyword arg
                        if self.check_ident("desc") || self.check_ident("nulls_first") {
                            break;
                        }
                        atoms.push(self.expect_atom("expected column atom")?);
                        self.skip_newlines();
                        if !self.match_token(&TokenKind::Comma) {
                            break;
                        }
                    }
                    // Parse optional keyword args: desc: true/false, nulls_first: true/false
                    while self.check_ident("desc") || self.check_ident("nulls_first") {
                        let kw = self.advance();
                        let kw_name = match &kw.kind {
                            TokenKind::Ident(s) => s.clone(),
                            _ => unreachable!(),
                        };
                        self.expect(&TokenKind::Colon, "expected ':' after keyword")?;
                        self.skip_newlines();
                        let val = self
                            .expect_bool_literal(&format!("expected true/false for {kw_name}"))?;
                        match kw_name.as_str() {
                            "desc" => descending = val,
                            "nulls_first" => nulls_first = val,
                            _ => unreachable!(),
                        }
                        self.skip_newlines();
                        if !self.match_token(&TokenKind::Comma) {
            ...
            DfVerbKind::Drop => {
                let atoms = self.atom_list()?;
                DfVerbArgs::Drop(atoms)
            }
            DfVerbKind::Rename => {
                let pairs = self.rename_pairs()?;
                DfVerbArgs::Rename(pairs)
            }
            DfVerbKind::Cast => {
                let atom = self.expect_atom("expected column atom in cast")?;
                self.expect(&TokenKind::Comma, "expected ',' after column in cast")?;
                self.skip_newlines();
                let type_name =
                    self.expect_upper_ident("expected type name (Int, Float, String, Bool)")?;
                DfVerbArgs::Cast(atom, type_name)
            }
            DfVerbKind::Head => {
                let n_expr = self.expression()?;
                DfVerbArgs::Head(Box::new(n_expr))
            }
            DfVerbKind::Tail => {
                let n_expr = self.expression()?;
                DfVerbArgs::Tail(Box::new(n_expr))
            }
            DfVerbKind::Slice => {
                let range_expr = self.expression()?;
                DfVerbArgs::Slice(Box::new(range_expr))
            }
            DfVerbKind::Count => {
                ...
                if !self.check(&TokenKind::RParen) {
                    loop {
                        self.skip_newlines();
                        if self.check_ident("sort") {
                            break;
                        }
                        columns.push(self.expect_atom("expected column atom in count")?);
                        self.skip_newlines();
                        if !self.match_token(&TokenKind::Comma) {
                            break;
                        }
                    ...
                    while self.check_ident("sort") {
                        self.advance(); // sort
                        self.expect(&TokenKind::Colon, "expected ':' after `sort`")?;
                        self.skip_newlines();
                        sort = self.expect_bool_literal("expected true/false for `sort`")?;
                        self.skip_newlines();
                        if !self.match_token(&TokenKind::Comma) {
                            break;
                        }
                ...
                DfVerbArgs::Count { columns, sort }
            }
            DfVerbKind::Pull => {
                let atom = self.expect_atom("expected column atom in pull")?;
                DfVerbArgs::Pull(atom)
            }
            DfVerbKind::Compute => DfVerbArgs::Compute,
            DfVerbKind::Collect => DfVerbArgs::Collect,
            DfVerbKind::Sample => {
                let mut n: Option<Box<Expr>> = None;
                let mut frac: Option<Box<Expr>> = None;
                if !self.check(&TokenKind::RParen) {
                    loop {
                        self.skip_newlines();
                        let key = self.expect_ident("expected `n` or `frac` in sample")?;
                        self.expect(&TokenKind::Colon, "expected ':' after sample key")?;
                        self.skip_newlines();
                        let value = Box::new(self.expression()?);
                        match key.node.as_str() {
                            "n" => {
                                if n.is_some() {
                                    self.error_at_current("duplicate `n:` in sample");
                                    return None;
                                }
                                n = Some(value);
                            }
                            "frac" => {
                                if frac.is_some() {
                                    self.error_at_current("duplicate `frac:` in sample");
                                    return None;
                                }
                                frac = Some(value);
                            }
                            _ => {
                                self.error_at_current(
                                    "unknown sample key (expected `n` or `frac`)",
                                );
                                return None;
                            }
                        }
            ...
            DfVerbKind::BindCols => {
                ...
                DfVerbArgs::BindCols(Box::new(right_expr))
            }
            DfVerbKind::Join => {
                // Parse: right_expr, [on: :key|[:k1,:k2]], [how: :left]
                let right_expr = self.expression()?;
                let mut on_keys: Option<Vec<Spanned<String>>> = None;
                let mut how: Option<Spanned<String>> = None;
                ...
                while self.match_token(&TokenKind::Comma) {
                    ...
                    if self.check_ident("on") {
                        if on_keys.is_some() {
                            self.error_at_current("duplicate `on:` in join");
                            return None;
                        }
                        self.advance(); // on
                        self.expect(&TokenKind::Colon, "expected ':' after `on`")?;
                        self.skip_newlines();
                        let keys = if self.check(&TokenKind::LBracket) {
                            self.advance(); // [
                            let atoms = self.atom_list()?;
                            self.expect(&TokenKind::RBracket, "expected ']' to close key list")?;
                            atoms
                        } else {
                            vec![self.expect_atom("expected column atom after `on:`")?]
                        };
                        on_keys = Some(keys);
                    } else if self.check_ident("how") {
                        if how.is_some() {
                            self.error_at_current("duplicate `how:` in join");
                            return None;
                        }
                        self.advance(); // how
                        self.expect(&TokenKind::Colon, "expected ':' after `how`")?;
                        self.skip_newlines();
                        how = Some(self.expect_atom("expected join kind atom after `how:`")?);
                    } else {
                        self.error_at_current("expected `on:` or `how:` in join");
                        return None;
                    }
                }
                ...
                DfVerbArgs::Join {
                    ...
                    on: on_keys.unwrap_or_default(),
                    how,
                }
            }
            DfVerbKind::Map => {
                // If the argument is a lambda, this is a List.map / function call,
                // not a DataFrame map verb. Return None to backtrack to regular call parsing.
                // Bare lambda: ident ->
                if let Some(TokenKind::Ident(_)) = self.peek_kind()
                    && self.peek_at(1).is_some_and(|t| t.kind == TokenKind::Arrow)
                {
                    return None;
                }
                // Parenthesized lambda: (params) -> expr
                if self.check(&TokenKind::LParen) && self.is_paren_lambda_start() {
                    return None;
                }
                let fn_expr = self.expression()?;
                DfVerbArgs::Map(Box::new(fn_expr))
            }
        };

        self.skip_newlines();
        let end = self.current_span();
        self.expect(&TokenKind::RParen, "expected ')' to close verb")?;

        Some(Spanned::new(
            ExprKind::DfVerb { verb, args },
            start.merge(end),
        ))
    }
    ...
    /// Parse mutate/summarize entries:
    /// - `name: col_expr`
    ...
    fn across_spec(&mut self) -> Option<DfColAssignment> {
        ...
        let selector = if self.check(&TokenKind::LBracket) {
            DfAcrossSelector::Columns(self.atom_bracket_list()?)
        } else {
            DfAcrossSelector::Selector(self.expect_type_param_name("expected across selector")?)
        };
        self.skip_newlines();
        self.expect(&TokenKind::Comma, "expected ',' after across selector")?;
        self.skip_newlines();
        let template = self.column_expr()?;
        self.skip_newlines();
        self.expect(&TokenKind::RParen, "expected ')' after across arguments")?;
        Some(DfColAssignment::Across(DfAcrossSpec { selector, template }))
    }
    ...
    fn atom_bracket_list(&mut self) -> Option<Vec<Spanned<String>>> {
        ...
        if !self.check(&TokenKind::RBracket) {
            loop {
                self.skip_newlines();
                atoms.push(self.expect_atom("expected :column atom in across selector")?);
                self.skip_newlines();
                if !self.match_token(&TokenKind::Comma) {
                    break;
                }
        ...
        self.skip_newlines();
        self.expect(&TokenKind::RBracket, "expected ']' to close atom list")?;
        Some(atoms)
    }
    ...
    fn rename_pairs(&mut self) -> Option<Vec<(Spanned<String>, Spanned<String>)>> {
        ...
        loop {
            ...
            self.expect(&TokenKind::Colon, "expected ':' after new name")?;
            self.skip_newlines();
            let old_name = self.expect_atom("expected :old_name atom")?;
            pairs.push((new_name, old_name));
            self.skip_newlines();
            if !self.match_token(&TokenKind::Comma) {
                break;
            }
    ...
    fn col_expr_pratt(&mut self, min_bp: u8) -> Option<ColExpr> {
        ...
        loop {
            ...
            let Some((l_bp, r_bp)) = self.col_expr_infix_bp() else {
                break;
            };
            if l_bp < min_bp {
                break;
            ...
            self.skip_newlines();
            let rhs = self.col_expr_pratt(r_bp)?;

            let span = lhs.span.merge(rhs.span);
            let op = if is_not_in {
                BinOp::NotIn
            } else if op_token.kind == TokenKind::In {
                BinOp::In
            } else {
                match token_to_binop(&op_token.kind) {
                    Some(op) => op,
                    ...
                    None => {
                        self.error_at_current("unsupported operator in column expression");
                        return None;
                    }
                }
            };
    ...
    fn col_expr_unary_or_primary(&mut self) -> Option<ColExpr> {
        ...
        if self.check(&TokenKind::Minus) {
            let op_token = self.advance();
            let operand = self.col_expr_pratt(17)?;
            let span = start.merge(operand.span);
            return Some(Spanned::new(
                ColExprKind::UnaryOp {
                    op: Spanned::new(UnaryOp::Neg, op_token.span),
        ...
        // Unary not (but NOT `not in` — that's handled as a compound infix)
        if self.check(&TokenKind::Not) && !self.peek_at(1).is_some_and(|t| t.kind == TokenKind::In)
        {
            let op_token = self.advance();
            let operand = self.col_expr_pratt(17)?;
            let span = start.merge(operand.span);
    ...
    fn col_expr_primary(&mut self) -> Option<ColExpr> {
        ...
        // ColumnExpr cond: cond { pred -> value, _ -> fallback }
        if self.check(&TokenKind::Cond) {
            return self.col_cond_expr();
        }
        ...
        if self.check(&TokenKind::Dollar) {
            ...
            self.error_at_current("expected variable name after '$'");
            return None;
        }
        ...
        if let Some(TokenKind::Int(n)) = self.peek_kind() {
            let n = *n;
            self.advance();
            return Some(Spanned::new(ColExprKind::Lit(Lit::Int(n)), start));
        }
        ...
        if let Some(TokenKind::Float(f)) = self.peek_kind() {
            let f = *f;
            self.advance();
            return Some(Spanned::new(ColExprKind::Lit(Lit::Float(f)), start));
        }
        ...
        if let Some(TokenKind::String(s)) = self.peek_kind() {
            let s = s.clone();
            self.advance();
            return Some(Spanned::new(ColExprKind::Lit(Lit::String(s)), start));
        }

        // Bool literals
        if self.check(&TokenKind::True) {
            self.advance();
            return Some(Spanned::new(ColExprKind::Lit(Lit::Bool(true)), start));
        }
        if self.check(&TokenKind::False) {
            self.advance();
            return Some(Spanned::new(ColExprKind::Lit(Lit::Bool(false)), start));
        }

        // None in ColumnExpr (KERNEL §8.2)
        if self.check(&TokenKind::NoneKw) {
            self.advance();
            return Some(Spanned::new(ColExprKind::None, start));
        }

        // Parenthesized column expression
        if self.check(&TokenKind::LParen) {
            self.advance();
            self.skip_newlines();
            let inner = self.column_expr()?;
            self.skip_newlines();
            self.expect(&TokenKind::RParen, "expected ')' in column expression")?;
            return Some(inner);
        }

        // Function call in ColumnExpr: func(args) — e.g. sum(:price), abs(:x)
        if let Some(TokenKind::Ident(name)) = self.peek_kind()
            && self.peek_is(|k| matches!(k, TokenKind::LParen))
        {
            let name = name.clone();
            let name_tok = self.advance();
            let func = Spanned::new(name, name_tok.span);
            self.advance(); // consume '('
            let mut args = Vec::new();
            self.skip_newlines();
            if !self.check(&TokenKind::RParen) {
        ...
        // List literal for `in` / `not in` RHS: [elem, elem, ...]
        if self.check(&TokenKind::LBracket) {
            self.advance(); // consume '['
            let mut items = Vec::new();
            self.skip_newlines();
            ...
            return Some(Spanned::new(ColExprKind::List(items), start.merge(end)));
        }

        self.error_at_current(
            "expected column expression (atom :name, literal, $capture, or function call)",
        );
        None
    }
    ...
    fn col_cond_arm(&mut self) -> Option<ColCondArm> {
        ...
        let condition = if let Some(TokenKind::Ident(name)) = self.peek_kind() {
            if name == "_" {
                let tok = self.advance();
                Spanned::new(ColExprKind::Wildcard, tok.span)
            } else {
                self.column_expr()?
            }
        } else {
...
fn verb_kind(name: &str) -> Option<DfVerbKind> {
    match name {
        ...
        "select" => Some(DfVerbKind::Select),
        "mutate" => Some(DfVerbKind::Mutate),
        "update" => Some(DfVerbKind::Update),
        "group_by" => Some(DfVerbKind::GroupBy),
        "summarize" => Some(DfVerbKind::Summarize),
        "sort" => Some(DfVerbKind::Sort),
        "distinct" => Some(DfVerbKind::Distinct),
        "drop" => Some(DfVerbKind::Drop),
        "rename" => Some(DfVerbKind::Rename),
        "cast" => Some(DfVerbKind::Cast),
        "head" => Some(DfVerbKind::Head),
        "tail" => Some(DfVerbKind::Tail),
        "slice" => Some(DfVerbKind::Slice),
        "count" => Some(DfVerbKind::Count),
        "pull" => Some(DfVerbKind::Pull),
        "compute" => Some(DfVerbKind::Compute),
        "collect" => Some(DfVerbKind::Collect),
        "sample" => Some(DfVerbKind::Sample),
        "bind_rows" => Some(DfVerbKind::BindRows),
        "bind_cols" => Some(DfVerbKind::BindCols),
        "join" => Some(DfVerbKind::Join),
...
mod tests {
    ...
    #[test]
    fn parse_filter_verb() {
        let expr = parse_df_verb("filter(:age > 28)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Filter);
                match args {
                    DfVerbArgs::Filter(col_expr) => match &col_expr.node {
                        ColExprKind::BinaryOp { op, left, right } => {
                            assert_eq!(op.node, BinOp::Gt);
                            assert_eq!(left.node, ColExprKind::Atom("age".into()));
                            assert_eq!(right.node, ColExprKind::Lit(Lit::Int(28)));
                        }
                        _ => panic!("expected BinaryOp, got {:?}", col_expr.node),
                    },
                    _ => panic!("expected Filter args"),
                }
            }
            _ => panic!("expected DfVerb, got {:?}", expr.node),
        }
    }

    #[test]
    fn parse_filter_complex_expr() {
        let expr = parse_df_verb("filter(:age > 28 and :active == true)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Filter);
                match args {
                    DfVerbArgs::Filter(col_expr) => match &col_expr.node {
                        ColExprKind::BinaryOp { op, .. } => {
    ...
    #[test]
    fn parse_filter_with_capture() {
        let expr = parse_df_verb("filter(:price > $threshold)");
        match &expr.node {
            ExprKind::DfVerb {
                args: DfVerbArgs::Filter(col_expr),
                ..
            } => match &col_expr.node {
                ColExprKind::BinaryOp { right, .. } => {
                    assert_eq!(right.node, ColExprKind::Capture("threshold".into()));
                }
                _ => panic!("expected BinaryOp"),
            },
            _ => panic!("expected DfVerb"),
    ...
    #[test]
    fn parse_select_verb() {
        let expr = parse_df_verb("select(:name, :age)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                ...
                match args {
                    DfVerbArgs::Select(atoms) => {
                        ...
                        assert_eq!(atoms[0].node, "name");
                        assert_eq!(atoms[1].node, "age");
                    }
                    _ => panic!("expected Select args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    #[test]
    fn parse_mutate_verb() {
        let expr = parse_df_verb("mutate(total: :price * :qty)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Mutate);
                match args {
                    DfVerbArgs::Mutate(items) => {
                        assert_eq!(items.len(), 1);
                        match &items[0] {
                            DfColAssignment::Named(name, expr) => {
                                assert_eq!(name.node, "total");
                                match &expr.node {
                                    ColExprKind::BinaryOp { op, .. } => {
                                        assert_eq!(op.node, BinOp::Mul);
                                    }
                            ...
                            other => panic!("expected named assignment, got {other:?}"),
                        }
                    }
                    _ => panic!("expected Mutate args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    #[test]
    fn parse_mutate_across_explicit_columns() {
        let expr = parse_df_verb("mutate(across([:price, :qty], round(:col, 2)))");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Mutate);
                match args {
                    DfVerbArgs::Mutate(items) => {
                        ...
                        match &items[0] {
                            DfColAssignment::Across(spec) => {
                                match &spec.selector {
                                    DfAcrossSelector::Columns(cols) => {
                                        assert_eq!(cols.len(), 2);
                                        assert_eq!(cols[0].node, "price");
                                        assert_eq!(cols[1].node, "qty");
                                    }
                                    other => panic!("expected columns selector, got {other:?}"),
                                }
                                match &spec.template.node {
                                    ColExprKind::Call { func, .. } => {
                                        assert_eq!(func.node, "round");
                                    }
                                    other => panic!("expected call template, got {other:?}"),
                                }
                            }
                            other => panic!("expected across assignment, got {other:?}"),
                        }
                    }
                    _ => panic!("expected Mutate args"),
                }
    ...
    #[test]
    fn parse_mutate_across_selector() {
        let expr = parse_df_verb("mutate(across(Numeric, round(:col, 2)))");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Mutate);
                match args {
                    DfVerbArgs::Mutate(items) => {
                        assert_eq!(items.len(), 1);
                        match &items[0] {
                            DfColAssignment::Across(spec) => match &spec.selector {
                                DfAcrossSelector::Selector(name) => {
                                    assert_eq!(name.node, "Numeric");
                                }
    ...
    #[test]
    fn parse_mutate_mixed_named_and_across() {
        let expr =
            parse_df_verb("mutate(total: :price * :qty, across(Orderable, coalesce(:col, :col)))");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Mutate);
                match args {
                    DfVerbArgs::Mutate(items) => {
                        ...
                        assert!(matches!(items[0], DfColAssignment::Named(_, _)));
                        assert!(matches!(items[1], DfColAssignment::Across(_)));
                    }
                    _ => panic!("expected Mutate args"),
                }
            }
    ...
    #[test]
    fn parse_sort_verb() {
        let expr = parse_df_verb("sort(:age)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Sort);
                match args {
                    DfVerbArgs::Sort {
                        columns,
                        descending,
                        ...
                        assert_eq!(columns.len(), 1);
                        assert_eq!(columns[0].node, "age");
                        assert!(!descending);
                        assert!(!nulls_first);
                    }
                    _ => panic!("expected Sort args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    #[test]
    fn parse_sort_verb_descending() {
        let expr = parse_df_verb("sort(:age, desc: true)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Sort);
                match args {
    ...
    #[test]
    fn parse_sort_verb_all_options() {
        let expr = parse_df_verb("sort(:a, :b, desc: true, nulls_first: true)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Sort);
                match args {
                    DfVerbArgs::Sort {
                        ...
                        assert_eq!(columns.len(), 2);
                        assert_eq!(columns[0].node, "a");
                        assert_eq!(columns[1].node, "b");
                        assert!(descending);
                        assert!(nulls_first);
                    }
                    _ => panic!("expected Sort args"),
    ...
    #[test]
    fn parse_distinct_verb() {
        let expr = parse_df_verb("distinct()");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Distinct);
                assert!(matches!(args, DfVerbArgs::Distinct));
            }
            _ => panic!("expected DfVerb"),
        }
    }
    ...
    #[test]
    fn parse_drop_verb() {
        let expr = parse_df_verb("drop(:temp, :internal)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Drop);
                match args {
                    DfVerbArgs::Drop(atoms) => {
                        assert_eq!(atoms.len(), 2);
                        assert_eq!(atoms[0].node, "temp");
                        assert_eq!(atoms[1].node, "internal");
                    }
                    _ => panic!("expected Drop args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    #[test]
    fn parse_group_by_verb() {
        let expr = parse_df_verb("group_by(:sector)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::GroupBy);
                match args {
                    DfVerbArgs::GroupBy(atoms) => {
                        assert_eq!(atoms.len(), 1);
                        assert_eq!(atoms[0].node, "sector");
                    }
                    _ => panic!("expected GroupBy args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    #[test]
    fn parse_summarize_verb() {
        let expr = parse_df_verb("summarize(avg_price: mean(:price))");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Summarize);
                match args {
                    DfVerbArgs::Summarize(items) => {
                        assert_eq!(items.len(), 1);
                        match &items[0] {
                            DfColAssignment::Named(name, expr) => {
                                assert_eq!(name.node, "avg_price");
                                match &expr.node {
                                    ColExprKind::Call { func, args } => {
                                        assert_eq!(func.node, "mean");
                                        assert_eq!(args.len(), 1);
                                        assert_eq!(args[0].node, ColExprKind::Atom("price".into()));
                                    }
                                    _ => panic!("expected Call, got {:?}", expr.node),
                                }
                            }
                            other => panic!("expected named summarize assignment, got {other:?}"),
                        }
                    }
                    _ => panic!("expected Summarize args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    #[test]
    fn parse_summarize_across_selector() {
        let expr = parse_df_verb("summarize(across(Numeric, mean(:col)))");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Summarize);
                match args {
                    DfVerbArgs::Summarize(items) => {
                        ...
                        match &items[0] {
                            ...
                            other => panic!("expected across summarize assignment, got {other:?}"),
                        }
                    }
                    _ => panic!("expected Summarize args"),
                }
    ...
    #[test]
    fn parse_update_verb() {
        let expr = parse_df_verb("update(:age, :age + 1)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Update);
                match args {
                    DfVerbArgs::Update(atom, col_expr) => {
                        assert_eq!(atom.node, "age");
                        match &col_expr.node {
                            ColExprKind::BinaryOp { op, .. } => {
                                assert_eq!(op.node, BinOp::Add);
                            }
    ...
    #[test]
    fn parse_rename_verb() {
        let expr = parse_df_verb("rename(new_name: :old_name)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Rename);
                match args {
                    DfVerbArgs::Rename(pairs) => {
                        assert_eq!(pairs.len(), 1);
                        assert_eq!(pairs[0].0.node, "new_name");
                        assert_eq!(pairs[0].1.node, "old_name");
                    }
                    _ => panic!("expected Rename args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    fn parse_col_expr_precedence() {
        ...
        match &expr.node {
            ExprKind::DfVerb {
                ...
                match &col_expr.node {
                    ColExprKind::BinaryOp { op, left, .. } => {
                        assert_eq!(op.node, BinOp::Gt);
                        // LHS should be :a + (:b * :c)
                        match &left.node {
                            ColExprKind::BinaryOp { op, right, .. } => {
                                assert_eq!(op.node, BinOp::Add);
                                match &right.node {
                                    ColExprKind::BinaryOp { op, .. } => {
                                        assert_eq!(op.node, BinOp::Mul);
    ...
    #[test]
    fn parse_col_expr_unary_neg() {
        let expr = parse_df_verb("filter(-:x > 0)");
        match &expr.node {
            ExprKind::DfVerb {
                args: DfVerbArgs::Filter(col_expr),
                ..
            } => match &col_expr.node {
                ColExprKind::BinaryOp { left, .. } => match &left.node {
                    ColExprKind::UnaryOp { op, operand } => {
                        assert_eq!(op.node, UnaryOp::Neg);
                        assert_eq!(operand.node, ColExprKind::Atom("x".into()));
                    }
                    _ => panic!("expected UnaryOp, got {:?}", left.node),
                },
                _ => panic!("expected BinaryOp"),
            },
            _ => panic!("expected DfVerb"),
    ...
    #[test]
    fn parse_col_expr_function_call() {
        let expr = parse_df_verb("filter(abs(:diff) > 10)");
        match &expr.node {
            ExprKind::DfVerb {
                args: DfVerbArgs::Filter(col_expr),
                ..
                ...
                ColExprKind::BinaryOp { left, .. } => match &left.node {
                    ColExprKind::Call { func, args } => {
                        assert_eq!(func.node, "abs");
                        assert_eq!(args.len(), 1);
                        assert_eq!(args[0].node, ColExprKind::Atom("diff".into()));
                    }
                    _ => panic!("expected Call, got {:?}", left.node),
                },
                _ => panic!("expected BinaryOp"),
            },
            _ => panic!("expected DfVerb"),
    ...
    #[test]
    fn parse_col_expr_parenthesized() {
        let expr = parse_df_verb("filter((:a + :b) * :c > 100)");
        match &expr.node {
            ExprKind::DfVerb {
                args: DfVerbArgs::Filter(col_expr),
                    ...
                    ColExprKind::BinaryOp { left, .. } => {
                        ...
                        match &left.node {
                            ColExprKind::BinaryOp {
                                ...
                                assert_eq!(op.node, BinOp::Mul);
                                // inner_left should be (:a + :b)
                                match &inner_left.node {
                                    ColExprKind::BinaryOp { op, .. } => {
                                        assert_eq!(op.node, BinOp::Add);
                                    }
    ...
    #[test]
    fn verb_name_as_variable_without_parens() {
        // When not followed by (, verb names should parse as regular variables
        let expr = parse("filter");
        assert_eq!(expr.node, ExprKind::Var("filter".into()));
    }

    #[test]
    fn verb_name_call_parses_as_regular_call() {
        let expr = parse("group_by(xs, key_fn)");
        match &expr.node {
            ExprKind::Call { func, args } => {
                assert_eq!(func.node, ExprKind::Var("group_by".into()));
                assert_eq!(args.len(), 2);
    ...
    #[test]
    fn parse_col_expr_string_literal() {
        let expr = parse_df_verb(r#"filter(:name == "Alice")"#);
        match &expr.node {
            ExprKind::DfVerb {
                args: DfVerbArgs::Filter(col_expr),
                ..
                ...
                ColExprKind::BinaryOp { op, right, .. } => {
                    assert_eq!(op.node, BinOp::Eq);
                    assert_eq!(right.node, ColExprKind::Lit(Lit::String("Alice".into())));
                }
                _ => panic!("expected BinaryOp"),
            },
            _ => panic!("expected DfVerb"),
    ...
    #[test]
    fn parse_col_expr_in_range_literal() {
        let expr = parse_df_verb("filter(:age in 18..=65)");
        match &expr.node {
            ExprKind::DfVerb {
                args: DfVerbArgs::Filter(col_expr),
                ..
                ...
                ColExprKind::BinaryOp { op, right, .. } => {
                    ...
                    match &right.node {
                        ColExprKind::Range {
                            ...
                            assert_eq!(start.node, ColExprKind::Lit(Lit::Int(18)));
                            assert_eq!(end.node, ColExprKind::Lit(Lit::Int(65)));
                            assert!(*inclusive);
                        }
                        other => panic!("expected ColExpr range, got {other:?}"),
                    }
                }
                _ => panic!("expected BinaryOp"),
            },
    ...
    #[test]
    fn parse_join_single_key() {
        let expr = parse_df_verb("join(df2, on: :id)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Join);
                match args {
                    DfVerbArgs::Join { on, .. } => {
                        assert_eq!(on.len(), 1);
                        assert_eq!(on[0].node, "id");
                    }
                    _ => panic!("expected Join args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    #[test]
    fn parse_join_multi_key() {
        let expr = parse_df_verb("join(df2, on: [:k1, :k2])");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Join);
                match args {
                    DfVerbArgs::Join { on, .. } => {
                        assert_eq!(on.len(), 2);
                        assert_eq!(on[0].node, "k1");
                        assert_eq!(on[1].node, "k2");
                    }
                    _ => panic!("expected Join args"),
                }
            }
    ...
    #[test]
    fn parse_join_how_left() {
        let expr = parse_df_verb("join(df2, on: :id, how: :left)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Join);
                match args {
                    DfVerbArgs::Join { on, how, .. } => {
                        assert_eq!(on.len(), 1);
                        assert_eq!(on[0].node, "id");
                        assert_eq!(how.as_ref().map(|h| h.node.as_str()), Some("left"));
                    }
                    _ => panic!("expected Join args"),
                }
            }
    ...
    #[test]
    fn parse_join_how_cross_without_on() {
        let expr = parse_df_verb("join(df2, how: :cross)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Join);
                match args {
                    DfVerbArgs::Join { on, how, .. } => {
                        assert!(on.is_empty());
                        assert_eq!(how.as_ref().map(|h| h.node.as_str()), Some("cross"));
                    }
                    _ => panic!("expected Join args"),
                }
            }
    ...
    #[test]
    fn parse_join_regular_call_not_df_verb() {
        let expr = parse("join(\",\", parts)");
        match &expr.node {
            ExprKind::Call { func, args } => {
                assert_eq!(func.node, ExprKind::Var("join".into()));
                assert_eq!(args.len(), 2);
            }
            _ => panic!("expected Call"),
        }
    ...
    #[test]
    fn parse_sample_with_n() {
        let expr = parse_df_verb("sample(n: 10)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::Sample);
                match args {
                    DfVerbArgs::Sample { n, frac } => {
                        assert!(n.is_some());
                        assert!(frac.is_none());
                    }
                    _ => panic!("expected Sample args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    #[test]
    fn parse_bind_rows_variadic() {
        let expr = parse_df_verb("bind_rows(df2, df3)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::BindRows);
                match args {
                    DfVerbArgs::BindRows(exprs) => {
                        assert_eq!(exprs.len(), 2);
                    }
                    _ => panic!("expected BindRows args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
    ...
    #[test]
    fn parse_bind_cols_single() {
        let expr = parse_df_verb("bind_cols(df2)");
        match &expr.node {
            ExprKind::DfVerb { verb, args } => {
                assert_eq!(verb.node, DfVerbKind::BindCols);
                match args {
                    DfVerbArgs::BindCols(_) => {}
                    _ => panic!("expected BindCols args"),
                }
            }
            _ => panic!("expected DfVerb"),
        }
