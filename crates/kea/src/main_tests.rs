    ...
    fn compile_and_execute_fip_unique_qualified_forwarder_with_ambiguous_short_names_exit_code() {
        ...
        let run = run_file(&source_path)
            .expect("@fip verifier should accept qualified safe forwarder calls when short names are ambiguous");
        assert_eq!(run.exit_code, 0);
    ...
    fn compile_fip_unique_module_alias_forwarder_call_ignores_local_operation_shadowing() {
        let project_dir =
            temp_workspace_project_dir("kea-cli-fip-unique-alias-opname-shadowed-param");
        let src_dir = project_dir.join("src");
        std::fs::create_dir_all(&src_dir).expect("source dir should be created");
        let source_path = src_dir.join("main.kea");
        std::fs::write(
    ...
    fn compile_rejects_fip_unique_shadowed_forwarder_name_call_escape() {
        ...
        let err = run_file(&source_path)
            .expect_err("@fip verifier should reject call-boundary escape through shadowed forwarder name");
        assert!(
