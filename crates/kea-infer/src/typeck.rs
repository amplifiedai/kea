...
use kea_ast::{
    AliasDecl, Argument, BinOp, EffectDecl, Expr, ExprKind, FnDecl,
...
pub fn validate_declared_fn_effect_row_with_env_and_records(
    ...
    if let Some(name) = effect_annotation_var_name(&declared) {
        ...
        // The polymorphic effect variable contract is validated by the main
        // unifier: if the function's resolved scheme has an open effect row
        // (rest variable), the variable propagates correctly.  No separate
        // trial-substitution pass needed.
        return Ok(());
    }
...
pub fn validate_trait_method_impl_contract_with_env(
    ...
    if let Some(name) = effect_annotation_var_name(&impl_ann.node) {
        ...
        // Polymorphic effect variable validated by main unifier — open row
        // means the variable propagates.
        return Ok(());
    }
    Err(Diagnostic::error(
        Category::TypeMismatch,
...
fn collect_resume_usage(
    ...
    match &expr.node {
        ...
        ExprKind::For(for_expr) => {
            collect_resume_usage(&for_expr.source, true, inside_lambda, resume_count, diagnostics);
            collect_resume_usage(&for_expr.body, true, inside_lambda, resume_count, diagnostics);
        }
        ExprKind::BinaryOp { left, right, .. } => {
    ...
    let ty = match &expr.node {
        ...
        ExprKind::For(_) => {
            unifier.push_error(
                Diagnostic::error(
                    Category::TypeError,
                    "`for` loops are not yet implemented (requires Iterable/Iterator traits from phase 0g)",
                )
                .at(span_to_loc(expr.span)),
            );
            Type::Unit
...
fn check_expr_bidir(
    ...
    match (&expr.node, expected) {
        ...
        (ExprKind::Block(exprs), _) => {
            ...
            env.push_scope();
            for current in &exprs[..exprs.len() - 1] {
