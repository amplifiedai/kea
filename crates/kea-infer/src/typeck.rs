...
pub struct TypeEnv {
    ...
    /// Populated by `init_session` from the stdlib registry.
    col_expr_type_rules: BTreeMap<String, kea_types::ColumnExprTypeRule>,
...
impl TypeEnv {
    pub fn new() -> Self {
        Self {
            ...
            col_expr_type_rules: kea_types::default_column_expr_type_rules(),
            stream_contexts: Vec::new(),
            actor_context_depth: 0,
        }
    }

    /// Set the column expression type rules (called once during session init).
    pub fn set_col_expr_type_rules(
        &mut self,
        rules: BTreeMap<String, kea_types::ColumnExprTypeRule>,
    ) {
        self.col_expr_type_rules = rules;
    }

    /// Look up a column expression type rule by function name.
    pub fn col_expr_type_rule(&self, name: &str) -> Option<&kea_types::ColumnExprTypeRule> {
        self.col_expr_type_rules.get(name)
    }
...
fn instantiate_impl_type(
    ...
    match ty {
        ...
        Type::DataFrame(inner) => Type::DataFrame(Box::new(instantiate_impl_type(
            inner,
            type_params,
            bindings,
        ))),
        Type::Column(inner) => Type::Column(Box::new(instantiate_impl_type(
            inner,
            type_params,
            bindings,
        ))),
        Type::GroupedFrame { row, keys } => Type::GroupedFrame {
            row: Box::new(instantiate_impl_type(row, type_params, bindings)),
            keys: keys.clone(),
        },
    ...
    match ann {
        ...
        TypeAnnotation::Applied(name, args) => {
            ...
            match (name.as_str(), args.as_slice()) {
                ...
                ("DataFrame", [schema]) => Some(Type::DataFrame(Box::new(
                    resolve_annotation_with_type_params(
                        schema,
                        type_param_scope,
                        records,
                ...
                ("Column", [inner]) => {
                    Some(Type::Column(Box::new(resolve_annotation_with_type_params(
                        inner,
                        type_param_scope,
                        records,
                        sum_types,
                ...
                ("GroupedFrame", [schema]) => Some(Type::GroupedFrame {
                    row: Box::new(resolve_annotation_with_type_params(
                        schema,
                        type_param_scope,
                        records,
                        sum_types,
...
fn rename_type(
    ...
    match ty {
        ...
        Type::DataFrame(inner) => {
            Type::DataFrame(Box::new(rename_type(inner, type_map, row_map, dim_map)))
        }
        Type::Column(inner) => {
            Type::Column(Box::new(rename_type(inner, type_map, row_map, dim_map)))
        }
        Type::Tagged { inner, tags } => Type::Tagged {
        ...
        Type::GroupedFrame { row, keys } => Type::GroupedFrame {
            row: Box::new(rename_type(row, type_map, row_map, dim_map)),
            keys: keys.clone(),
        },
        Type::Stream(inner) => {
...
// ---------------------------------------------------------------------------
// DataFrame type-level operations (Phase 2 — KERNEL §7)
// ---------------------------------------------------------------------------

/// Type-level `expect_schema`: narrows `DataFrame(Dynamic)` to `DataFrame(R)`.
///
/// Returns `Result(DataFrame(R), SchemaError)` where R is the expected schema.
/// Errors if the input is not `DataFrame(Dynamic)`.
pub fn df_expect_schema(
    input_ty: &Type,
    expected_row: RowType,
    unifier: &mut Unifier,
    span: Span,
) -> Type {
    let prov = Provenance {
    ...
    // Input must be DataFrame(Dynamic)
    let expected_input = Type::DataFrame(Box::new(Type::Dynamic));
    constrain_type_eq(unifier, input_ty, &expected_input, &prov);

    // Output: Result(DataFrame(R), SchemaError)
    let schema_ty = Type::AnonRecord(expected_row);
    Type::Result(
        Box::new(Type::DataFrame(Box::new(schema_ty))),
        Box::new(Type::String), // SchemaError placeholder until error types are defined
    )
}

fn parse_decimal_schema_type_name(s: &str) -> Option<Type> {
    let inner = s.strip_prefix("Decimal(")?.strip_suffix(')')?;
    let (precision, scale) = inner.split_once(',')?;
    let precision = precision.trim().parse::<i64>().ok()?;
    let scale = scale.trim().parse::<i64>().ok()?;
    Some(Type::Decimal {
...
fn parse_schema_type_name(s: &str) -> Option<Type> {
    if let Some(base) = s.strip_suffix('?') {
        return parse_schema_type_name(base).map(|inner| Type::Option(Box::new(inner)));
    }
    if let Some(decimal) = parse_decimal_schema_type_name(s) {
        return Some(decimal);
    }
    match s {
        "Int" => Some(Type::Int),
        "Int8" => Some(Type::IntN(IntWidth::I8, Signedness::Signed)),
        "Int16" => Some(Type::IntN(IntWidth::I16, Signedness::Signed)),
        "Int32" => Some(Type::IntN(IntWidth::I32, Signedness::Signed)),
        "Int64" => Some(Type::IntN(IntWidth::I64, Signedness::Signed)),
        "UInt8" => Some(Type::IntN(IntWidth::I8, Signedness::Unsigned)),
        "UInt16" => Some(Type::IntN(IntWidth::I16, Signedness::Unsigned)),
        "UInt32" => Some(Type::IntN(IntWidth::I32, Signedness::Unsigned)),
        "UInt64" => Some(Type::IntN(IntWidth::I64, Signedness::Unsigned)),
        "Float" => Some(Type::Float),
        "Float16" => Some(Type::FloatN(FloatWidth::F16)),
        "Float32" => Some(Type::FloatN(FloatWidth::F32)),
        "Float64" => Some(Type::FloatN(FloatWidth::F64)),
        "Bool" => Some(Type::Bool),
...
fn parse_schema_row_literal(expr: &Expr, unifier: &mut Unifier) -> Option<RowType> {
    let ExprKind::AnonRecord { fields, spread } = &expr.node else {
        unifier.push_error(
            Diagnostic::error(
                Category::TypeMismatch,
                "schema argument must be an anonymous record of type-name strings, for example `#{ id: \"Int\", name: \"String\" }`",
            )
    ...
    if spread.is_some() {
        unifier.push_error(
            Diagnostic::error(
                Category::TypeMismatch,
                "schema argument may not use spread (`..`) — list all expected columns explicitly",
            )
            .at(span_to_loc(expr.span)),
        );
        return None;
    ...
    for (name, value_expr) in fields {
        if !seen.insert(name.node.clone()) {
            unifier.push_error(
                Diagnostic::error(
                    ...
                    format!("duplicate schema field `{}`", name.node),
                )
                .at(span_to_loc(name.span)),
            );
            continue;
        ...
        let ExprKind::Lit(Lit::String(type_name)) = &value_expr.node else {
            unifier.push_error(
                Diagnostic::error(
                    Category::TypeMismatch,
                    format!(
                        "schema field `{}` must be a type-name string literal, got non-string expression",
                        name.node
                    ),
                )
        ...
        let Some(col_ty) = parse_schema_type_name(type_name) else {
            unifier.push_error(
                Diagnostic::error(
                    Category::TypeMismatch,
                    format!(
                        "unknown schema type `{}` for field `{}`",
                        type_name, name.node
                    ),
                )
                .at(span_to_loc(value_expr.span))
                .with_help(
                    "supported schema type names: Int, Float, Bool, String, Date, DateTime, Decimal(p, s), and precision variants".to_string(),
                ),
            );
            continue;
        };

        parsed_fields.push((Label::new(name.node.clone()), col_ty));
    }

    Some(RowType::closed(parsed_fields))
}

/// Check if a call is `expect_type(expr)` and handle it specially.
///
/// `expect_type` narrows a Dynamic value to a concrete type at runtime.
/// The target type is inferred from bidirectional type context (e.g.,
/// `let x: Int = expect_type(dyn_val)?`).
///
/// Returns `Result(T, String)` where T is the target type.
#[allow(clippy::too_many_arguments)]
fn infer_expect_type_call(
    func: &Expr,
    bound_args: &[BoundArg],
    env: &mut TypeEnv,
    unifier: &mut Unifier,
    ...
    // Only intercept bare `expect_type` calls
    let ExprKind::Var(name) = &func.node else {
        return None;
    };
    if name != "expect_type" {
        return None;
    ...
    // Infer argument type — must be Dynamic
    let arg_ty = match &bound_args[0] {
        BoundArg::Provided(arg_expr) | BoundArg::Default(arg_expr) => {
            infer_expr_bidir(arg_expr, env, unifier, records, traits, sum_types)
        }
    };
    let prov = Provenance {
        span: call_span,
    ...
    // Constrain input to Dynamic (widening direction: expected=Dynamic, actual=arg)
    constrain_type_eq(unifier, &Type::Dynamic, &arg_ty, &prov);

    // Determine target type from bidirectional context.
    // Priority: explicit expected_context > bidir_expected > fresh var.
    let context = expected_context
        .cloned()
        .or_else(|| unifier.bidir_expected().cloned());
    let target_ty = if let Some(expected) = context {
        let resolved = unifier.substitution.apply(&expected);
        match &resolved {
            Type::Result(inner, _) => (**inner).clone(),
            _ => resolved,
        }
    } else {
        // No bidir context — use a fresh type var.
        // If it remains unresolved, the caller will get Result(a, String)
        // which is fine for type checking but won't have runtime target info.
        unifier.fresh_type()
    };

    // Record target type for evaluator
    let resolved_target = unifier.substitution.apply(&target_ty);
    let target_name = kea_types::sanitize_type_display(&resolved_target);
    unifier
        .type_annotations
        .expect_type_targets
        .insert(call_span, target_name);

    Some(Type::Result(
        Box::new(target_ty),
        Box::new(Type::String), // TypeError placeholder
    ))
}

fn io_reader_name(func: &Expr) -> Option<&str> {
    match &func.node {
        ExprKind::Var(name) if matches!(name.as_str(), "read_csv" | "read_parquet") => {
            Some(name.as_str())
        }
        ExprKind::FieldAccess { field, .. }
            if matches!(field.node.as_str(), "read_csv" | "read_parquet") =>
        {
            Some(field.node.as_str())
        }
        _ => None,
    }
}
...
#[allow(clippy::too_many_arguments)]
fn infer_io_reader_schema_call(
    func: &Expr,
    bound_args: &[BoundArg],
    ...
    let _reader_name = io_reader_name(func)?;
    if bound_args.len() != 2 {
        return None;
    }
    ...
    let path_ty = match &bound_args[0] {
        BoundArg::Provided(path_expr) => {
            infer_expr_bidir(path_expr, env, unifier, records, traits, sum_types)
        }
        BoundArg::Default(path_expr) => infer_expr_bidir(
            path_expr,
            &mut default_env,
            unifier,
            records,
    ...
    constrain_type_eq(
        ...
        &Type::String,
        &Provenance {
            span: bound_arg_expr(&bound_args[0]).span,
            reason: Reason::FunctionArg { param_index: 0 },
        },
    );
    ...
    {
        default_env.bind(param_name.clone(), TypeScheme::mono(path_ty.clone()));
    }

    let schema_expr = bound_arg_expr(&bound_args[1]);
    match &bound_args[1] {
        BoundArg::Provided(_) => {
            let _ = infer_expr_bidir(schema_expr, env, unifier, records, traits, sum_types);
        }
        BoundArg::Default(_) => {
            let _ = infer_expr_bidir(
                schema_expr,
                &mut default_env,
                unifier,
                records,
    ...
    let schema_row =
        parse_schema_row_literal(schema_expr, unifier).unwrap_or_else(|| RowType::closed(vec![]));

    let io_error_ty = kea_types::builtin_error_sum_type("IOError").unwrap_or(Type::String);
    Some(Type::Result(
        Box::new(Type::DataFrame(Box::new(Type::AnonRecord(schema_row)))),
        Box::new(io_error_ty),
    ))
}

/// Type-level `mutate`: adds a new column to a DataFrame.
///
/// Given `DataFrame({a: Int | r})` where `r` lacks `:b`, produces
/// `DataFrame({a: Int, b: T | r})`. Returns the new DataFrame type.
/// Uses the unifier to enforce lacks constraints (no duplicate columns).
pub fn df_mutate(
    input_ty: &Type,
    column: Label,
    value_ty: Type,
    ...
    let prov = Provenance {
        span,
        reason: Reason::DataFrameMutate {
            column: column.clone(),
        },
    };

    // Input must be DataFrame(AnonRecord(row))
    let input_row_var = unifier.fresh_row_var();
    let input_row = RowType::open(vec![], input_row_var);
    let expected_input = Type::DataFrame(Box::new(Type::AnonRecord(input_row.clone())));
    constrain_type_eq(unifier, input_ty, &expected_input, &prov);

    // Check if the column already exists in the resolved input row.
    // This enforces the lacks constraint for mutate (can't add a duplicate column).
    let resolved_input = unifier.substitution.apply(input_ty);
    if let Type::DataFrame(inner) = &resolved_input
        && let Type::AnonRecord(row) = inner.as_ref()
        && row.fields.iter().any(|(l, _)| l == &column)
    {
        unifier.push_error(
            Diagnostic::error(
                Category::LacksViolation,
                format!(
                    "cannot add column `{column}` — DataFrame already has a column named `{column}`"
                ),
            )
            .with_help("use `update` to change an existing column's values"),
        );
    }

    // Build output row: extend with the new column.
    let output_row = RowType::open(vec![(column, value_ty)], input_row_var);
    Type::DataFrame(Box::new(Type::AnonRecord(output_row)))
}

/// Type-level `update`: replaces a column's values in-place (same type).
///
/// Given `DataFrame({a: Int | r})`, updating `:a` with `Int` succeeds.
/// Updating `:a` with `String` is a type error.
pub fn df_update(
    input_ty: &Type,
    column: Label,
    value_ty: &Type,
    ...
    let prov = Provenance {
        span,
        reason: Reason::DataFrameUpdate {
            column: column.clone(),
        },
    };

    // Input must be DataFrame with a row containing this column
    let col_ty = unifier.fresh_type();
    let rest = unifier.fresh_row_var();
    let input_row = RowType::open(vec![(column, col_ty.clone())], rest);
    let expected_input = Type::DataFrame(Box::new(Type::AnonRecord(input_row)));
    constrain_type_eq(unifier, input_ty, &expected_input, &prov);

    // The new value must match the existing column type
    constrain_type_eq(unifier, &col_ty, value_ty, &prov);

    // Output type is same as input (schema unchanged)
    unifier.substitution.apply(input_ty)
}

/// Type-level `drop`: removes a column from a DataFrame.
///
/// Given `DataFrame({a: Int, b: String | r})`, dropping `:a` produces
/// `DataFrame({b: String | r})`.
pub fn df_drop(input_ty: &Type, column: Label, unifier: &mut Unifier, span: Span) -> Type {
    let prov = Provenance {
        span,
        reason: Reason::DataFrameDrop {
            column: column.clone(),
        },
    };
    ...
    let col_ty = unifier.fresh_type();
    let rest = unifier.fresh_row_var();
    let input_row = RowType::open(vec![(column, col_ty)], rest);
    let expected_input = Type::DataFrame(Box::new(Type::AnonRecord(input_row)));
    constrain_type_eq(unifier, input_ty, &expected_input, &prov);

    // Output is the rest (without the dropped column)
    let rest_row = unifier.substitution.apply_row(&RowType::open(vec![], rest));
    Type::DataFrame(Box::new(Type::AnonRecord(rest_row)))
}

fn df_replace_column(
    input_ty: &Type,
    column: &Label,
    value_ty: Type,
    unifier: &mut Unifier,
    ...
    let resolved = unifier.substitution.apply(input_ty);
    if let Some(fields) = extract_df_fields(&resolved)
        && fields.iter().any(|(label, _)| label == column)
    {
        let without = df_drop(&resolved, column.clone(), unifier, span);
        df_mutate(&without, column.clone(), value_ty, unifier, span)
    } else {
        df_mutate(&resolved, column.clone(), value_ty, unifier, span)
    }
}

fn substitute_across_placeholder(expr: &ColExpr, column_name: &str) -> ColExpr {
    fn recurse(expr: &ColExpr, column_name: &str) -> ColExpr {
        let node = match &expr.node {
            ColExprKind::Atom(name) if name == "col" => ColExprKind::Atom(column_name.to_string()),
            ColExprKind::Atom(name) => ColExprKind::Atom(name.clone()),
            ColExprKind::Lit(lit) => ColExprKind::Lit(lit.clone()),
            ColExprKind::BinaryOp { op, left, right } => ColExprKind::BinaryOp {
                op: op.clone(),
                left: Box::new(recurse(left, column_name)),
                right: Box::new(recurse(right, column_name)),
            },
            ColExprKind::UnaryOp { op, operand } => ColExprKind::UnaryOp {
                op: op.clone(),
                operand: Box::new(recurse(operand, column_name)),
            },
            ColExprKind::Capture(name) => ColExprKind::Capture(name.clone()),
            ColExprKind::Call { func, args } => ColExprKind::Call {
                func: func.clone(),
                args: args.iter().map(|arg| recurse(arg, column_name)).collect(),
            },
            ColExprKind::Cond { arms } => ColExprKind::Cond {
                arms: arms
                    .iter()
                    .map(|arm| ColCondArm {
                        condition: Box::new(recurse(&arm.condition, column_name)),
                        body: Box::new(recurse(&arm.body, column_name)),
                        span: arm.span,
                    })
                    .collect(),
            },
            ColExprKind::None => ColExprKind::None,
            ColExprKind::Wildcard => ColExprKind::Wildcard,
            ColExprKind::List(items) => ColExprKind::List(
                items
                    .iter()
                    .map(|item| recurse(item, column_name))
                    .collect(),
            ),
            ColExprKind::Range {
                start,
                ...
                start: Box::new(recurse(start, column_name)),
                end: Box::new(recurse(end, column_name)),
                inclusive: *inclusive,
            },
        };
    ...
    recurse(expr, column_name)
}

fn across_selector_matches_type(
    selector: &str,
    ty: &Type,
    traits: &TraitRegistry,
    span: Span,
    ...
    match selector {
        ...
        "Orderable" => {
            matches!(ty, Type::Bool | Type::String | Type::Date | Type::DateTime) || ty.is_numeric()
        }
        "Display" => true,
        "Int" => ty.is_integer(),
        "Float" => ty.is_float(),
        "Decimal" => ty.is_decimal(),
        "Bool" => matches!(ty, Type::Bool),
        "String" => matches!(ty, Type::String),
        "Date" => matches!(ty, Type::Date),
        "DateTime" => matches!(ty, Type::DateTime),
        other => match solve_trait_impl_with_diag(traits, unifier, other, ty, span) {
...
fn select_across_columns(
    df_ty: &Type,
    selector: &DfAcrossSelector,
    unifier: &mut Unifier,
    traits: &TraitRegistry,
    span: Span,
) -> Vec<(Label, Type)> {
    let resolved = unifier.substitution.apply(df_ty);
    let Some(fields) = extract_df_fields(&resolved) else {
        return Vec::new();
    };
    match selector {
        DfAcrossSelector::Columns(columns) => columns
            .iter()
            .filter_map(|col| {
                let label = Label::new(col.node.clone());
                let ty = fields
                    .iter()
                ...
                if let Some(ty) = ty {
                    ...
                    unifier.push_error(
                        Diagnostic::error(
                            Category::TypeError,
                            format!("across column `:{}` not found in DataFrame", col.node),
                        )
                        .at(span_to_loc(span)),
                    );
                    None
            ...
            .collect(),
        DfAcrossSelector::Selector(name) => {
            ...
            if !known_builtin && traits.lookup_trait(&name.node).is_none() {
                unifier.push_error(
                    Diagnostic::error(
                        Category::TraitBound,
                        format!("unknown across selector `{}`", name.node),
                    )
                    .at(span_to_loc(name.span))
                    .with_help(
                        "use a known selector (Numeric, Orderable, Int, Float, String, ...) or a registered trait name",
                    ),
                );
                return Vec::new();
            }
            fields
                .iter()
                .filter(|(_label, field_ty)| {
                    across_selector_matches_type(&name.node, field_ty, traits, name.span, unifier)
                })
                .cloned()
                .collect()
        }
...
// ---------------------------------------------------------------------------
// DataFrame verb type inference
// ---------------------------------------------------------------------------

/// Infer the result type of a DataFrame verb applied to an input DataFrame.
///
/// This is called when the pipe `|>` RHS is a `DfVerb`. The `input_ty` is the
/// type of the LHS (must be a DataFrame).
#[allow(clippy::too_many_arguments)]
fn infer_df_verb(
    input_ty: &Type,
    verb: &kea_ast::Spanned<DfVerbKind>,
    args: &DfVerbArgs,
    env: &mut TypeEnv,
    unifier: &mut Unifier,
    span: Span,
    ...
    let prov = |reason| Provenance { span, reason };

    // GroupBy and Summarize have special input requirements.
    // All other verbs require a DataFrame (not GroupedFrame).
    match &verb.node {
        DfVerbKind::GroupBy => {
            // GroupBy requires DataFrame input — verify it
            let row_var = unifier.fresh_row_var();
            let expected_df =
                Type::DataFrame(Box::new(Type::AnonRecord(RowType::open(vec![], row_var))));
            constrain_type_eq(unifier, input_ty, &expected_df, &prov(Reason::ColumnExpr));
        }
        DfVerbKind::Summarize => {
            // Summarize requires GroupedFrame — checked in the match arm below
        }
        _ => {
            // All other verbs require DataFrame (not GroupedFrame)
            let resolved = unifier.substitution.apply(input_ty);
            if matches!(&resolved, Type::GroupedFrame { .. }) {
                unifier.push_error(
                    Diagnostic::error(
                        Category::TypeError,
                        format!(
                            "`{}` cannot be used on a GroupedFrame; use `summarize` to aggregate or apply this verb before `group_by`",
    ...
    match (&verb.node, args) {
        (DfVerbKind::Filter, DfVerbArgs::Filter(col_expr)) => {
            let col_ty = infer_col_expr(col_expr, input_ty, env, unifier, span);
            let inner = unwrap_col(&col_ty);
            // Accept Bool or Bool? (None rows dropped per KERNEL §10.4)
            let base = match &inner {
                Type::Option(inner) => inner.as_ref(),
                other => other,
            };
            constrain_type_eq(unifier, base, &Type::Bool, &prov(Reason::FilterPredicate));
            // Filter preserves the schema
            unifier.substitution.apply(input_ty)
        }

        (DfVerbKind::Select, DfVerbArgs::Select(atoms)) => {
            // Verify each atom exists in the schema and build a restricted row
            let mut selected_fields = Vec::new();
            for atom in atoms {
                let col_label = Label::new(atom.node.clone());
                let col_ty = resolve_atom(&col_label, input_ty, unifier, span);
                selected_fields.push((col_label, col_ty));
            }
            let selected_row = RowType::closed(selected_fields);
            Type::DataFrame(Box::new(Type::AnonRecord(selected_row)))
        }

        (DfVerbKind::Mutate, DfVerbArgs::Mutate(items)) => {
            let mut result_ty = unifier.substitution.apply(input_ty);
            for item in items {
                match item {
                    DfColAssignment::Named(name, col_expr) => {
                        let col_label = Label::new(name.node.clone());
                        let col_ty = infer_col_expr(col_expr, &result_ty, env, unifier, span);
                        let bare_ty = unwrap_col(&col_ty);
                        result_ty = df_mutate(&result_ty, col_label, bare_ty, unifier, span);
                    }
                    DfColAssignment::Across(spec) => {
                        let selected = select_across_columns(
                            &result_ty,
                            &spec.selector,
                            unifier,
                            traits,
                        ...
                        if selected.is_empty() {
                            unifier.push_error(
                                Diagnostic::warning(
                                    Category::TypeError,
                                    "across selector matched no DataFrame columns",
                                )
                                .at(span_to_loc(span)),
                            );
                        }
                        for (label, _field_ty) in selected {
                            let expanded = substitute_across_placeholder(&spec.template, &label.0);
                            let col_ty = infer_col_expr(&expanded, &result_ty, env, unifier, span);
                            let bare_ty = unwrap_col(&col_ty);
                            // across mutates existing columns in-place; keep schema width stable.
                            result_ty =
                                df_replace_column(&result_ty, &label, bare_ty, unifier, span);
                        }
                    }
                }
            }
        ...
        (DfVerbKind::Update, DfVerbArgs::Update(atom, col_expr)) => {
            let col_label = Label::new(atom.node.clone());
            let col_ty = infer_col_expr(col_expr, input_ty, env, unifier, span);
            let bare_ty = unwrap_col(&col_ty);
            df_update(input_ty, col_label, &bare_ty, unifier, span)
        }

        (DfVerbKind::Sort, DfVerbArgs::Sort { columns, .. }) => {
            // Verify each sort column exists; schema is preserved
            for atom in columns {
                let col_label = Label::new(atom.node.clone());
                resolve_atom(&col_label, input_ty, unifier, span);
            }
            unifier.substitution.apply(input_ty)
        }

        (DfVerbKind::Distinct, DfVerbArgs::Distinct) => {
            // Schema preserved
            unifier.substitution.apply(input_ty)
        }

        (DfVerbKind::Drop, DfVerbArgs::Drop(atoms)) => {
            let mut result_ty = unifier.substitution.apply(input_ty);
            for atom in atoms {
                let col_label = Label::new(atom.node.clone());
                result_ty = df_drop(&result_ty, col_label, unifier, span);
            }
            result_ty
        }

        (DfVerbKind::GroupBy, DfVerbArgs::GroupBy(atoms)) => {
            let mut key_labels = Vec::new();
            for atom in atoms {
                let col_label = Label::new(atom.node.clone());
                resolve_atom(&col_label, input_ty, unifier, span);
                key_labels.push(col_label);
            }
            Type::GroupedFrame {
                row: Box::new(unifier.substitution.apply(input_ty)),
                keys: key_labels,
            }
        }

        (DfVerbKind::Summarize, DfVerbArgs::Summarize(items)) => {
            // Summarize requires GroupedFrame input
            let resolved = unifier.substitution.apply(input_ty);
            let (inner_df_ty, key_labels) = match &resolved {
                Type::GroupedFrame { row, keys } => (row.as_ref().clone(), keys.clone()),
                _ => {
                    unifier.push_error(
                        Diagnostic::error(
                            Category::TypeError,
                            "`summarize` requires a GroupedFrame; add `group_by` before `summarize`",
                        )
                        .at(SourceLocation {
                            file_id: span.file.0,
                            start: span.start,
                    ...
                    // Fall back: treat as ungrouped summarize
                    ...
                    for item in items {
                        match item {
                            DfColAssignment::Named(name, col_expr) => {
                                ...
                                let col_ty = infer_col_expr(col_expr, input_ty, env, unifier, span);
                                let bare_ty = unwrap_col(&col_ty);
                                result_fields.push((col_label, bare_ty));
                            }
                            DfColAssignment::Across(spec) => {
                                let selected = select_across_columns(
                                    input_ty,
                                ...
                                for (label, _field_ty) in selected {
                                    let expanded =
                                        substitute_across_placeholder(&spec.template, &label.0);
                                    let col_ty =
                                        infer_col_expr(&expanded, input_ty, env, unifier, span);
                                    let bare_ty = unwrap_col(&col_ty);
                                    result_fields.push((label, bare_ty));
                                }
                            }
                        }
                    }
                    let result_row = RowType::closed(result_fields);
                    return Type::DataFrame(Box::new(Type::AnonRecord(result_row)));
                }
            };

            // Build result: group keys first, then aggregation columns
            let mut result_fields = Vec::new();
            for key in &key_labels {
                let key_ty = resolve_atom(key, &inner_df_ty, unifier, span);
                result_fields.push((key.clone(), key_ty));
            }
            for item in items {
                match item {
                    DfColAssignment::Named(name, col_expr) => {
                        let col_label = Label::new(name.node.clone());
                        let col_ty = infer_col_expr(col_expr, &inner_df_ty, env, unifier, span);
                        let bare_ty = unwrap_col(&col_ty);
                        result_fields.push((col_label, bare_ty));
                    }
                    DfColAssignment::Across(spec) => {
                        ...
                        for (label, _field_ty) in selected {
                            let expanded = substitute_across_placeholder(&spec.template, &label.0);
                            let col_ty =
                                infer_col_expr(&expanded, &inner_df_ty, env, unifier, span);
                            let bare_ty = unwrap_col(&col_ty);
                            result_fields.push((label, bare_ty));
                        }
                    }
            ...
            let result_row = RowType::closed(result_fields);
            Type::DataFrame(Box::new(Type::AnonRecord(result_row)))
        }

        (DfVerbKind::Rename, DfVerbArgs::Rename(pairs)) => {
            // For each rename pair: verify old column exists, drop it, add new name
            let mut result_ty = unifier.substitution.apply(input_ty);
            for (new_name, old_name) in pairs {
                let old_label = Label::new(old_name.node.clone());
                let new_label = Label::new(new_name.node.clone());
                let col_ty = resolve_atom(&old_label, &result_ty, unifier, span);
                result_ty = df_drop(&result_ty, old_label, unifier, span);
                result_ty = df_mutate(&result_ty, new_label, col_ty, unifier, span);
            }
            result_ty
        }

        (DfVerbKind::Cast, DfVerbArgs::Cast(col_name, target_type_name)) => {
            let label = Label::new(col_name.node.clone());
            // Verify column exists (ignore its current type)
            let _ = resolve_atom(&label, input_ty, unifier, span);
            // Parse target type name
            let target_ty =
                parse_cast_target_type_name(target_type_name.node.as_str()).unwrap_or_else(|| {
                    let other = target_type_name.node.as_str();
                    unifier.push_error(
                        Diagnostic::error(
                            Category::TypeError,
                            format!(
                                "unknown cast target type `{other}`; expected scalar cast target (Int/Float variants, Decimal(p, s), String, or Bool)"
                            ),
                        )
                        .at(SourceLocation {
                            file_id: span.file.0,
                            start: target_type_name.span.start,
                            end: target_type_name.span.end,
                        }),
                    );
                    unifier.fresh_type()
                });
            // Drop old column type and add back with target type
            let without = df_drop(input_ty, label.clone(), unifier, span);
            df_mutate(&without, label, target_ty, unifier, span)
        }

        (DfVerbKind::Head, DfVerbArgs::Head(n_expr)) => {
            // Infer the argument and require an integer count; schema is preserved.
            let n_ty = infer_expr_bidir(n_expr, env, unifier, records, traits, sum_types);
            let resolved_n_ty = unifier.substitution.apply(&n_ty);
            if matches!(resolved_n_ty, Type::Var(_)) {
                // Keep conservative defaulting behavior for unresolved literals/vars.
                constrain_type_eq(
                    unifier,
                    &n_ty,
                    &Type::Int,
            ...
            } else if !resolved_n_ty.is_integer() {
                unifier.push_error(
                    Diagnostic::error(
                        Category::TypeError,
                        format!("`head` expects an integer count, but got `{resolved_n_ty}`"),
                    )
                    .at(span_to_loc(n_expr.span)),
                );
            }
            unifier.substitution.apply(input_ty)
        }
        (DfVerbKind::Tail, DfVerbArgs::Tail(n_expr)) => {
            let n_ty = infer_expr_bidir(n_expr, env, unifier, records, traits, sum_types);
            let resolved_n_ty = unifier.substitution.apply(&n_ty);
            if matches!(resolved_n_ty, Type::Var(_)) {
                constrain_type_eq(
                ...
                unifier.push_error(
                    Diagnostic::error(
                        Category::TypeError,
                        format!("`tail` expects an integer count, but got `{resolved_n_ty}`"),
                    )
                    .at(span_to_loc(n_expr.span)),
                );
            }
        ...
        (DfVerbKind::Slice, DfVerbArgs::Slice(range_expr)) => {
            let range_ty = infer_expr_bidir(range_expr, env, unifier, records, traits, sum_types);
            constrain_type_eq(
                unifier,
                &range_ty,
                &Type::Opaque {
        ...
        (DfVerbKind::Count, DfVerbArgs::Count { columns, .. }) => {
            let resolved_input = unifier.substitution.apply(input_ty);
            let (input_fields, group_keys) = match &resolved_input {
                Type::DataFrame(inner) => (
                    extract_df_fields(inner).unwrap_or_default(),
                    Vec::<Label>::new(),
                ),
                Type::GroupedFrame { row, keys } => {
                    (extract_df_fields(row).unwrap_or_default(), keys.clone())
                }
                _ => {
                    let df_row_var = unifier.fresh_row_var();
                    constrain_type_eq(
                        unifier,
                        input_ty,
                        &Type::DataFrame(Box::new(Type::AnonRecord(RowType::open(
                            vec![],
                            df_row_var,
                        )))),
                        &prov(Reason::FunctionArg { param_index: 0 }),
            ...
            let key_labels: Vec<Label> = if columns.is_empty() {
                group_keys
            } else {
                columns.iter().map(|a| Label::new(a.node.clone())).collect()
            };
            ...
            for key in &key_labels {
                ...
                if let Some(ty) = key_ty {
                    result_fields.push((key.clone(), ty));
                } else {
                    unifier.push_error(
                        Diagnostic::error(
                            Category::TypeError,
                            format!("count key `{key}` not found in DataFrame"),
                        )
                        .at(span_to_loc(span)),
                    );
                }
            }
            result_fields.push((Label::new("n".to_string()), Type::Int));
            Type::DataFrame(Box::new(Type::AnonRecord(RowType::closed(result_fields))))
        }
        (DfVerbKind::Pull, DfVerbArgs::Pull(column)) => {
            let label = Label::new(column.node.clone());
            let col_ty = resolve_atom(&label, input_ty, unifier, span);
            Type::List(Box::new(col_ty))
        }
        (DfVerbKind::Sample, DfVerbArgs::Sample { n, frac }) => {
            match (n, frac) {
                (Some(_), Some(_)) => {
                    unifier.push_error(
                        Diagnostic::error(
                            Category::TypeError,
                            "`sample` accepts exactly one of `n:` or `frac:`",
                        )
                        .at(span_to_loc(span)),
                    );
                }
                (None, None) => {
                    unifier.push_error(
                        Diagnostic::error(
                            Category::TypeError,
                            "`sample` requires either `n:` or `frac:`",
                        )
                        .at(span_to_loc(span)),
                    );
                }
                    ...
                    if matches!(resolved_n_ty, Type::Var(_)) {
                        ...
                        unifier.push_error(
                            Diagnostic::error(
                                Category::TypeError,
                                format!(
                                    "`sample(n:)` expects an integer count, got `{resolved_n_ty}`"
                                ),
                            )
                            .at(span_to_loc(n_expr.span)),
                        );
                ...
                (None, Some(frac_expr)) => {
                    let frac_ty =
                        infer_expr_bidir(frac_expr, env, unifier, records, traits, sum_types);
                    let resolved_frac_ty = unifier.substitution.apply(&frac_ty);
                    if matches!(resolved_frac_ty, Type::Var(_)) {
                        constrain_type_eq(
                            unifier,
                            &frac_ty,
                            &Type::Float,
                    ...
                    } else if !resolved_frac_ty.is_float() {
                        unifier.push_error(
                            Diagnostic::error(
                                Category::TypeError,
                                format!(
                                    "`sample(frac:)` expects a floating-point fraction, got `{resolved_frac_ty}`"
                                ),
                            )
                            .at(span_to_loc(frac_expr.span)),
                        );
                    }
                }
            }
        ...
        (DfVerbKind::BindRows, DfVerbArgs::BindRows(exprs)) => {
            if exprs.is_empty() {
                unifier.push_error(
                    Diagnostic::error(
                        Category::TypeError,
                ...
                return unifier.substitution.apply(input_ty);
            }

            let mut result_ty = unifier.substitution.apply(input_ty);
            for (index, expr) in exprs.iter().enumerate() {
                let right_ty = infer_expr_bidir(expr, env, unifier, records, traits, sum_types);
                constrain_type_eq(
                    unifier,
                    &right_ty,
                    &result_ty,
                    &prov(Reason::FunctionArg { param_index: index }),
                );
                result_ty = unifier.substitution.apply(&result_ty);
            }

            result_ty
        }
        (DfVerbKind::BindCols, DfVerbArgs::BindCols(right_expr)) => {
            let right_ty = infer_expr_bidir(right_expr, env, unifier, records, traits, sum_types);

            let right_row_var = unifier.fresh_row_var();
            constrain_type_eq(
                unifier,
                &right_ty,
                &Type::DataFrame(Box::new(Type::AnonRecord(RowType::open(
            ...
            let left_resolved = unifier.substitution.apply(input_ty);
            let right_resolved = unifier.substitution.apply(&right_ty);
            let (left_fields, right_fields) = match (
                extract_df_fields(&left_resolved),
                extract_df_fields(&right_resolved),
            ) {
                (Some(left), Some(right)) => (left, right),
                _ => return unifier.fresh_type(),
            };

            let mut result_fields = left_fields.clone();
            for (label, ty) in right_fields {
                if left_fields
                    .iter()
                    .any(|(left_label, _)| left_label == &label)
                {
                    unifier.push_error(
                        Diagnostic::error(
                            Category::TypeError,
                            format!(
                                "bind_cols: overlapping column `{label}`; rename/drop columns before binding"
                            ),
                        )
                        .at(span_to_loc(span)),
                    );
                } else {
                    result_fields.push((label, ty));
                }
            }

            Type::DataFrame(Box::new(Type::AnonRecord(RowType::closed(result_fields))))
        }
        (DfVerbKind::Compute, DfVerbArgs::Compute) | (DfVerbKind::Collect, DfVerbArgs::Collect) => {
            unifier.substitution.apply(input_ty)
        }
        (DfVerbKind::Join, DfVerbArgs::Join { right, on, how }) => {
            let join_mode = how.as_ref().map(|h| h.node.as_str()).unwrap_or("inner");
            let is_cross = join_mode == "cross";
            if on.is_empty() && !is_cross {
                unifier.push_error(
                    Diagnostic::error(
                        Category::TypeError,
                        "join: `on:` is required unless `how: :cross`",
                    )
                    .at(span_to_loc(span)),
                );
            }
            if !on.is_empty() && is_cross {
                unifier.push_error(
                    Diagnostic::error(
                        Category::TypeError,
                        "join: `on:` must be omitted when `how: :cross`",
                    )
                    .at(span_to_loc(span)),
                );
            }
            if !matches!(
                ...
                unifier.push_error(
                    Diagnostic::error(
                        Category::TypeError,
                        format!(
                            "join: unknown `how` value `:{join_mode}` (expected :inner, :left, :right, :outer, :semi, :anti, or :cross)"
                        ),
                    )
                    .at(span_to_loc(how.as_ref().map(|h| h.span).unwrap_or(span))),
                );
            }

            // Infer the right DataFrame's type
            let right_ty = infer_expr_bidir(right, env, unifier, records, traits, sum_types);
            // Constrain it to be a DataFrame
            let right_row_var = unifier.fresh_row_var();
            let right_row = RowType::open(vec![], right_row_var);
            let expected_right = Type::DataFrame(Box::new(Type::AnonRecord(right_row)));
            constrain_type_eq(
                unifier,
                &right_ty,
                &expected_right,
            ...
            // Resolve both sides to concrete types
            let left_resolved = unifier.substitution.apply(input_ty);
            let right_resolved = unifier.substitution.apply(&right_ty);

            // Extract fields from both DataFrames
            let left_fields = extract_df_fields(&left_resolved);
            let right_fields = extract_df_fields(&right_resolved);

            let (left_fields, right_fields) = match (left_fields, right_fields) {
                (Some(l), Some(r)) => (l, r),
                _ => {
                    // Can't resolve fields — return a fresh type
                    return unifier.fresh_type();
                }
            };

            // For each key: verify it exists in both sides and types match
            let mut key_fields: Vec<(Label, Type)> = Vec::new();
            if !is_cross {
                for key_atom in on {
                    let key_label = Label::new(key_atom.node.clone());
                    let left_key_ty = left_fields
                        .iter()
                        .find(|(l, _)| l == &key_label)
                        .map(|(_, t)| t.clone());
                    let right_key_ty = right_fields
                        .iter()
                        .find(|(l, _)| l == &key_label)
                        .map(|(_, t)| t.clone());

                    match (left_key_ty, right_key_ty) {
                        (Some(lt), Some(rt)) => {
                            constrain_type_eq(unifier, &lt, &rt, &prov(Reason::ColumnExpr));
                            key_fields.push((key_label, unifier.substitution.apply(&lt)));
                        }
                        (None, _) => {
                            unifier.push_error(
                                Diagnostic::error(
                                    Category::TypeError,
                                    format!("join key `{key_label}` not found in left DataFrame"),
                                )
                                .at(SourceLocation {
                                    file_id: key_atom.span.file.0,
                                    start: key_atom.span.start,
                                    end: key_atom.span.end,
                                }),
                            );
                        }
                        (_, None) => {
                            unifier.push_error(
                                Diagnostic::error(
                                    Category::TypeError,
                                    format!("join key `{key_label}` not found in right DataFrame"),
                                )
                                .at(SourceLocation {
                                    file_id: key_atom.span.file.0,
                                    start: key_atom.span.start,
            ...
            let on_labels: Vec<Label> = on.iter().map(|a| Label::new(a.node.clone())).collect();

            // Left non-key columns
            let left_non_key: Vec<(Label, Type)> = left_fields
                .iter()
                .filter(|(l, _)| !on_labels.contains(l))
                .cloned()
                .collect();

            // Right non-key columns
            let right_non_key: Vec<(Label, Type)> = right_fields
                .iter()
                .filter(|(l, _)| !on_labels.contains(l))
                .cloned()
                .collect();

            if !matches!(join_mode, "semi" | "anti") {
                // Check for non-key column overlap (compile error)
                for (right_label, _) in &right_non_key {
                    if left_non_key.iter().any(|(l, _)| l == right_label) {
                        unifier.push_error(
                            Diagnostic::error(
                                Category::TypeError,
                                format!(
                                    "join: both DataFrames have non-key column `{right_label}`; rename or select to resolve the overlap"
                                ),
                            )
                            .at(SourceLocation {
                                file_id: span.file.0,
            ...
            // Build result: key columns + left non-key + right non-key
            let mut result_fields = Vec::new();
            result_fields.extend(key_fields);
            match join_mode {
                "left" => {
                    result_fields.extend(left_non_key);
                    for (label, ty) in right_non_key {
                        result_fields.push((label, Type::Option(Box::new(ty))));
                    }
                }
                "right" => {
                    for (label, ty) in left_non_key {
                        result_fields.push((label, Type::Option(Box::new(ty))));
                    }
                    result_fields.extend(right_non_key);
                }
                "outer" => {
                    for (label, ty) in left_non_key {
                        result_fields.push((label, Type::Option(Box::new(ty))));
        ...
        (DfVerbKind::Map, DfVerbArgs::Map(fn_expr)) => {
            // map(f) applies a row→row function to each row of the DataFrame.
            // The function takes the DataFrame's row type and returns a new row type.
            let fn_ty = infer_expr_bidir(fn_expr, env, unifier, records, traits, sum_types);

            // Build expected: (input_row) -> output_row
            let input_param = unifier.fresh_type();
            let output_ret = unifier.fresh_type();
            let expected_fn = Type::Function(FunctionType {
                params: vec![input_param.clone()],
                ret: Box::new(output_ret.clone()),
            });
            constrain_type_eq(unifier, &fn_ty, &expected_fn, &prov(Reason::MapFunction));

            // Unify the function's input with the DataFrame's inner row type
            if let Type::DataFrame(inner) = input_ty {
                constrain_type_eq(unifier, &input_param, inner, &prov(Reason::MapInput));
            }

            // Result: DataFrame(output_row)
            Type::DataFrame(Box::new(unifier.substitution.apply(&output_ret)))
        }

        _ => {
            // Mismatched verb/args — should not happen if parser is correct
            unifier.push_error(
                Diagnostic::error(
                    Category::TypeError,
                    "internal error: mismatched verb kind and arguments",
                )
                .at(SourceLocation {
                    file_id: span.file.0,
                    start: span.start,
...
fn verb_kind_name(kind: &DfVerbKind) -> &'static str {
    match kind {
        DfVerbKind::Filter => "filter",
        DfVerbKind::Select => "select",
        DfVerbKind::Mutate => "mutate",
        DfVerbKind::Update => "update",
        DfVerbKind::GroupBy => "group_by",
        DfVerbKind::Summarize => "summarize",
        DfVerbKind::Sort => "sort",
        DfVerbKind::Distinct => "distinct",
        DfVerbKind::Drop => "drop",
        DfVerbKind::Rename => "rename",
        DfVerbKind::Cast => "cast",
        DfVerbKind::Head => "head",
        DfVerbKind::Tail => "tail",
        DfVerbKind::Slice => "slice",
        DfVerbKind::Count => "count",
        DfVerbKind::Pull => "pull",
        DfVerbKind::Compute => "compute",
        DfVerbKind::Collect => "collect",
        DfVerbKind::Sample => "sample",
        DfVerbKind::BindRows => "bind_rows",
        DfVerbKind::BindCols => "bind_cols",
        DfVerbKind::Join => "join",
        DfVerbKind::Map => "map",
    }
}

/// Extract field list from a resolved DataFrame type.
///
/// Given `DataFrame(AnonRecord(RowType { fields, ... }))`, returns the fields.
fn extract_df_fields(ty: &Type) -> Option<Vec<(Label, Type)>> {
...
fn parse_decimal_cast_target_type_name(name: &str) -> Option<Type> {
...
fn parse_cast_target_type_name(name: &str) -> Option<Type> {
    match name {
        "Int" => Some(Type::Int),
        "Int8" => Some(Type::IntN(
        ...
        "String" => Some(Type::String),
        "Bool" => Some(Type::Bool),
        other => parse_decimal_cast_target_type_name(other),
    }
}
...
/// Resolve an atom (`:name`) against a DataFrame's row type, returning the column's type.
///
/// If the column doesn't exist, pushes an error and returns a fresh type variable.
fn resolve_atom(label: &Label, df_ty: &Type, unifier: &mut Unifier, span: Span) -> Type {
    let prov = Provenance {
        span,
        reason: Reason::DataFrameUpdate {
            column: label.clone(),
    ...
    // Constrain: the DataFrame must have a row with this column
    let col_ty = unifier.fresh_type();
    let rest = unifier.fresh_row_var();
    let expected_row = RowType::open(vec![(label.clone(), col_ty.clone())], rest);
    let expected_df = Type::DataFrame(Box::new(Type::AnonRecord(expected_row)));
    constrain_type_eq(unifier, df_ty, &expected_df, &prov);

    unifier.substitution.apply(&col_ty)
}

/// Unwrap `Column(T)` to bare `T` at verb boundaries.
///
/// After migration, the `other` arm should be unreachable — all `infer_col_expr`
/// results are `Column(T)`. The fallthrough exists for error recovery only.
fn unwrap_col(ty: &Type) -> Type {
    match ty {
        Type::Column(inner) => *inner.clone(),
        other => {
            debug_assert!(false, "unwrap_col called on non-Column type: {other}");
            other.clone()
        }
    }
}

/// Unwrap `Option(T)` if present. Returns `(base_type, was_nullable)`.
fn col_unwrap_option(ty: &Type) -> (&Type, bool) {
    match ty {
...
/// Wrap a type in `Option` if not already nullable.
fn col_make_nullable(ty: Type) -> Type {
    match ty {
        Type::Option(_) => ty,
...
fn known_dim_value(dim: &Dim) -> Option<i64> {
    match dim {
...
fn decimal_known_precision_scale(ty: &Type) -> Option<(i64, i64)> {
    let Type::Decimal { precision, scale } = ty else {
        return None;
    };
    Some((known_dim_value(precision)?, known_dim_value(scale)?))
}

fn fresh_decimal_type(unifier: &mut Unifier) -> Type {
    Type::Decimal {
        precision: unifier.fresh_dim(),
        scale: unifier.fresh_dim(),
    }
}

fn infer_decimal_binary_result(op: BinOp, left_ty: &Type, right_ty: &Type) -> Option<Type> {
    let (p1, s1) = decimal_known_precision_scale(left_ty)?;
    let (p2, s2) = decimal_known_precision_scale(right_ty)?;

    let result = match op {
        BinOp::Add | BinOp::Sub => Type::Decimal {
            precision: Dim::Known(std::cmp::max(p1, p2).saturating_add(1)),
            scale: Dim::Known(std::cmp::max(s1, s2)),
        },
        BinOp::Mul => Type::Decimal {
            precision: Dim::Known(p1.saturating_add(p2)),
            scale: Dim::Known(s1.saturating_add(s2)),
        },
        BinOp::Div => {
            let scale = std::cmp::max(6, s1);
            Type::Decimal {
                precision: Dim::Known(p1.saturating_add(s2).saturating_add(scale)),
                scale: Dim::Known(scale),
            }
        }
        _ => return None,
...
fn infer_decimal_binary_result_fallback(
    ...
    match (left_ty, right_ty) {
        (Type::Decimal { .. }, Type::Decimal { .. }) => {
            infer_decimal_binary_result(op, left_ty, right_ty)
                .or_else(|| Some(fresh_decimal_type(unifier)))
        }
        _ => None,
    }
}

/// Apply a declarative ColumnExpr type rule to produce a `Column(T)` result type.
///
/// This replaces the 96-line hardcoded match in earlier versions. Each rule
/// declaratively describes the type behavior (identity, fixed output, aggregate, etc.)
/// and this function applies it given the actual argument types.
fn apply_column_expr_type_rule(
    rule: &kea_types::ColumnExprTypeRule,
    func_name: &str,
    arg_inners: &[Type],
    unifier: &mut Unifier,
    prov: &Provenance,
    ...
    use kea_types::{AggregateResultKind, ColumnExprArgConstraint, ColumnExprTypeRule};

    let col = |inner: Type| Type::Column(Box::new(inner));

    let err_arity = |expected: usize, got: usize, u: &mut Unifier| {
        u.push_error(
            Diagnostic::error(
                Category::TypeError,
                format!("`{func_name}` takes exactly {expected} argument(s), got {got}"),
            )
            .at(loc),
        );
    };
    ...
    match rule {
        ColumnExprTypeRule::ScalarIdentity => {
            if arg_inners.len() != 1 {
                err_arity(1, arg_inners.len(), unifier);
                return col(unifier.fresh_type());
            }
            col(arg_inners[0].clone())
        }
        ColumnExprTypeRule::ScalarFixed(output_ty) => {
            if arg_inners.len() != 1 {
                err_arity(1, arg_inners.len(), unifier);
                return col(unifier.fresh_type());
            }
            let (_, nullable) = col_unwrap_option(&arg_inners[0]);
            let result = if nullable {
                col_make_nullable(output_ty.clone())
            } else {
                output_ty.clone()
            };
            col(result)
        }
        ColumnExprTypeRule::ScalarChecked { input, output } => {
            if arg_inners.len() != 1 {
                err_arity(1, arg_inners.len(), unifier);
                return col(output.clone());
            }
            let (base, nullable) = col_unwrap_option(&arg_inners[0]);
            constrain_type_eq(unifier, base, input, prov);
            let result = if nullable {
                col_make_nullable(output.clone())
            } else {
                output.clone()
            };
            col(result)
        }
        ColumnExprTypeRule::Aggregate => {
            if arg_inners.len() != 1 {
                err_arity(1, arg_inners.len(), unifier);
                return col(unifier.fresh_type());
            }
            let (base, _) = col_unwrap_option(&arg_inners[0]);
            col(col_make_nullable(base.clone()))
        }
        ColumnExprTypeRule::AggregateFixed(output_ty) => {
            if arg_inners.len() != 1 {
                err_arity(1, arg_inners.len(), unifier);
            }
            col(Type::Option(Box::new(output_ty.clone())))
        }
        ColumnExprTypeRule::AggregateCount => {
            // count takes 1 arg, count_all takes 0
            let expected = if func_name == "count_all" { 0 } else { 1 };
            if arg_inners.len() != expected {
                err_arity(expected, arg_inners.len(), unifier);
            }
            col(Type::Int)
        }
        ColumnExprTypeRule::AggregateConstrained { args, result } => {
            if arg_inners.len() != args.len() {
                err_arity(args.len(), arg_inners.len(), unifier);
                return col(unifier.fresh_type());
            }

            let mut bases = Vec::with_capacity(arg_inners.len());
            for (idx, arg_ty) in arg_inners.iter().enumerate() {
                let (base, _) = col_unwrap_option(arg_ty);
                bases.push(base.clone());
                match &args[idx] {
                    ColumnExprArgConstraint::Any => {}
                    ColumnExprArgConstraint::Exact(expected) => {
                        constrain_type_eq(unifier, base, expected, prov);
                    }
                    ColumnExprArgConstraint::Trait(trait_name) => {
                        let tv = unifier.fresh_type_var();
                        constrain_type_eq(unifier, base, &Type::Var(tv), prov);
                        constrain_trait_obligation(unifier, &Type::Var(tv), trait_name, prov);
                    }
                }
            }

            match result {
                AggregateResultKind::Input => {
                    let result = bases
                        .first()
                        .cloned()
                        .unwrap_or_else(|| unifier.fresh_type());
                    col(Type::Option(Box::new(result)))
                }
                AggregateResultKind::Fixed(output_ty) => {
                    col(Type::Option(Box::new(output_ty.clone())))
                }
                AggregateResultKind::Count => col(Type::Int),
            }
        }
        ColumnExprTypeRule::NullCheck => {
            if arg_inners.len() != 1 {
                err_arity(1, arg_inners.len(), unifier);
            }
            col(Type::Bool)
        }
        ColumnExprTypeRule::Coalesce => {
            if arg_inners.len() != 2 {
                err_arity(2, arg_inners.len(), unifier);
                return col(unifier.fresh_type());
            }
            let (first_base, _) = col_unwrap_option(&arg_inners[0]);
            let (second_base, second_nullable) = col_unwrap_option(&arg_inners[1]);
            constrain_type_eq(unifier, first_base, second_base, prov);
            let result = if second_nullable {
                col_make_nullable(second_base.clone())
            } else {
                second_base.clone()
            };
            col(result)
        }
        ColumnExprTypeRule::NullIf => {
            if arg_inners.len() != 2 {
                err_arity(2, arg_inners.len(), unifier);
                return col(unifier.fresh_type());
            }
            let (left_base, _) = col_unwrap_option(&arg_inners[0]);
            let (right_base, _) = col_unwrap_option(&arg_inners[1]);
            constrain_type_eq(unifier, left_base, right_base, prov);
            col(col_make_nullable(left_base.clone()))
        }
        ColumnExprTypeRule::WindowRowNumber => {
            if !arg_inners.is_empty() {
                err_arity(0, arg_inners.len(), unifier);
            }
            col(Type::Int)
        }
        ColumnExprTypeRule::WindowRankByArg { output } => {
            if arg_inners.len() != 1 {
                err_arity(1, arg_inners.len(), unifier);
                return col(unifier.fresh_type());
            }
            ...
            let tv = unifier.fresh_type_var();
            constrain_type_eq(unifier, base, &Type::Var(tv), prov);
            constrain_trait_obligation(unifier, &Type::Var(tv), "Orderable", prov);
            col(output.clone())
        }
        ColumnExprTypeRule::WindowNtile => {
            if arg_inners.len() != 1 {
                err_arity(1, arg_inners.len(), unifier);
                return col(unifier.fresh_type());
            }
            let (base, _) = col_unwrap_option(&arg_inners[0]);
            constrain_type_eq(unifier, base, &Type::Int, prov);
            col(Type::Int)
        }
        ColumnExprTypeRule::WindowOffset => {
            if !(1..=3).contains(&arg_inners.len()) {
                unifier.push_error(
                    Diagnostic::error(
                        Category::TypeError,
                        format!(
                            "`{func_name}` takes 1 to 3 argument(s), got {}",
                            arg_inners.len()
                        ),
                    )
                    .at(loc),
            ...
            let (value_base, _) = col_unwrap_option(&arg_inners[0]);
            if arg_inners.len() >= 2 {
                let (offset_base, _) = col_unwrap_option(&arg_inners[1]);
                constrain_type_eq(unifier, offset_base, &Type::Int, prov);
            }
            if arg_inners.len() >= 3 {
                let (default_base, _) = col_unwrap_option(&arg_inners[2]);
                constrain_type_eq(unifier, value_base, default_base, prov);
            }
            col(Type::Option(Box::new(value_base.clone())))
        }
    }
}

/// Infer the type of a column expression in the context of a DataFrame.
///
/// All results are wrapped in `Type::Column(T)` — the boundary between
/// columnar computation and scalar computation (KERNEL §7.6). At verb
/// boundaries (filter, mutate, etc.), `Column(T)` is unwrapped to bare `T`.
///
/// Nullable propagation: if either operand of a binary op is nullable (`T?`),
/// the result is nullable. This mirrors DataFusion's SQL null semantics.
fn infer_col_expr(
    col_expr: &ColExpr,
    df_ty: &Type,
    env: &TypeEnv,
    ...
    match &col_expr.node {
        ColExprKind::Atom(name) => {
            let label = Label::new(name.clone());
            let elem_ty = resolve_atom(&label, df_ty, unifier, span);
            col(elem_ty)
        }
        ...
        ColExprKind::None => {
            let fresh = unifier.fresh_type();
            col(Type::Option(Box::new(fresh)))
        }

        ColExprKind::BinaryOp { op, left, right } => {
            let left_col = infer_col_expr(left, df_ty, env, unifier, span);
            let left_inner = unwrap_col(&left_col);
            let (left_base, left_nullable) = col_unwrap_option(&left_inner);

            match op.node {
                // Membership: :col in [list] / :col not in [list] → Bool
                // RHS is a list literal or range literal; element/bounds type
                // must match LHS.
                BinOp::In | BinOp::NotIn => {
                    let right_ty = infer_col_expr(right, df_ty, env, unifier, span);
                    let right_resolved = unifier.substitution.apply(&right_ty);
                    let right_inner = match &right_resolved {
                        // Error recovery path: if a plain column slipped through, unwrap it.
                        Type::Column(inner) => unifier.substitution.apply(inner),
                        other => other.clone(),
                    };
                    match &right_inner {
                        Type::List(elem_ty) => {
                        ...
                        _ => {
                            unifier.push_error(
                                Diagnostic::error(
                                    Category::TypeError,
                                    format!(
                                        "`in` in column expressions expects a List or Range on the right, got {}",
                                        sanitize_type_display(&right_inner)
                                    ),
                                )
                                .at(loc()),
                            );
                    ...
                    // RHS element type should unify with LHS column base type.
                    // The result is always Bool (nullable if the column is nullable).
                    let result = if left_nullable {
                        col_make_nullable(Type::Bool)
                    } else {
                        Type::Bool
                    };
                    col(result)
                }
                // Arithmetic: unify base types, propagate nullable
                BinOp::Add | BinOp::Sub | BinOp::Mul | BinOp::Div | BinOp::Mod => {
                    let right_col = infer_col_expr(right, df_ty, env, unifier, span);
                    let right_inner = unwrap_col(&right_col);
                    let (right_base, right_nullable) = col_unwrap_option(&right_inner);
                    let either_nullable = left_nullable || right_nullable;
                    let left_resolved = unifier.substitution.apply(left_base);
                    let right_resolved = unifier.substitution.apply(right_base);
                    let arithmetic_result = if let Some(decimal_ty) =
                        infer_decimal_binary_result_fallback(
                            op.node,
                            &left_resolved,
                            &right_resolved,
                            unifier,
                    ...
                    let result = if either_nullable {
                        col_make_nullable(arithmetic_result)
                    } else {
                        arithmetic_result
                    };
                    col(result)
                }
                // Comparison: unify base types, result is Bool (nullable if either operand is)
                BinOp::Eq | BinOp::Neq | BinOp::Lt | BinOp::Lte | BinOp::Gt | BinOp::Gte => {
                    let right_col = infer_col_expr(right, df_ty, env, unifier, span);
                    let right_inner = unwrap_col(&right_col);
                    let (right_base, right_nullable) = col_unwrap_option(&right_inner);
                    let either_nullable = left_nullable || right_nullable;
                    // Ban == None / != None — use is_none()/is_some() instead
                    if matches!(op.node, BinOp::Eq | BinOp::Neq)
                        && (matches!(left.node, ColExprKind::None)
                            || matches!(right.node, ColExprKind::None))
                    {
                        let suggestion = if matches!(op.node, BinOp::Eq) {
                            "is_none"
                        } else {
                            "is_some"
                        };
                        unifier.push_error(
                            Diagnostic::error(
                                Category::TypeError,
                                format!(
                                    "use `{suggestion}()` instead of comparing with None; \
                                     null equality in column expressions follows SQL three-valued logic"
                                ),
                            )
                            .at(loc()),
                        );
                    ...
                    constrain_type_eq(unifier, left_base, right_base, &prov(Reason::ColumnExpr));
                    let result = if either_nullable {
                        col_make_nullable(Type::Bool)
                    } else {
                        Type::Bool
                ...
                // Logical: both must be Bool (or Bool?), result propagates nullable
                BinOp::And | BinOp::Or => {
                    let right_col = infer_col_expr(right, df_ty, env, unifier, span);
                    let right_inner = unwrap_col(&right_col);
                    let (right_base, right_nullable) = col_unwrap_option(&right_inner);
                    let either_nullable = left_nullable || right_nullable;
                    constrain_type_eq(unifier, left_base, &Type::Bool, &prov(Reason::ColumnExpr));
                    constrain_type_eq(unifier, right_base, &Type::Bool, &prov(Reason::ColumnExpr));
                    let result = if either_nullable {
                        col_make_nullable(Type::Bool)
                    } else {
                        Type::Bool
                ...
                // ++ and <> are not valid in ColumnExpr
                BinOp::Concat | BinOp::Combine => {
                    let op_name = if op.node == BinOp::Concat {
                        "++"
                    } else {
                        "<>"
                    };
                    unifier.push_error(
                        Diagnostic::error(
                            Category::TypeError,
                            format!(
                                "operator `{op_name}` is not supported in column expressions"
                            ),
                        )
                        .at(loc()),
                    );
        ...
        ColExprKind::UnaryOp { op, operand } => {
            let operand_col = infer_col_expr(operand, df_ty, env, unifier, span);
            let operand_inner = unwrap_col(&operand_col);
            let (operand_base, nullable) = col_unwrap_option(&operand_inner);

            match op.node {
                kea_ast::UnaryOp::Neg => {
                    let result = if nullable {
                        col_make_nullable(operand_base.clone())
                    } else {
                        operand_base.clone()
                    };
                    col(result)
        ...
        ColExprKind::Capture(name) => {
            if let Some(scheme) = env.lookup(name) {
                let inner = instantiate_with_span(scheme, unifier, col_expr.span);
                col(inner)
            } else {
                unifier.push_error(
                    Diagnostic::error(
                        Category::UndefinedName,
                        format!("capture `${name}` not found in scope"),
                    )
                    .at(SourceLocation {
                        file_id: span.file.0,
                        start: col_expr.span.start,
                        end: col_expr.span.end,
                    }),
                );
                col(unifier.fresh_type())
            }
        }

        ColExprKind::Call { func, args } => {
            // Infer types of all arguments (each is Column(T)).
            let col_arg_types: Vec<Type> = args
                .iter()
                .map(|arg| infer_col_expr(arg, df_ty, env, unifier, span))
                .collect();

            // Unwrap Column from args to get element types for function logic.
            let arg_inners: Vec<Type> = col_arg_types.iter().map(unwrap_col).collect();

            // Dispatch via declarative type rules from the TypeEnv registry.
            if let Some(rule) = env.col_expr_type_rule(&func.node) {
                apply_column_expr_type_rule(
                    rule,
                    &func.node,
                    &arg_inners,
                    unifier,
            ...
            } else if let Some(effects) = env.function_effect(&func.node) {
                if !effects.is_column_expr_eligible() {
                    // Impure function — can't be used in ColumnExpr
                    unifier.push_error(
                        Diagnostic::error(
                            Category::PurityViolation,
                            format!(
                                "function `{}` is impure and cannot be used in a column expression",
                                func.node
                            ),
                        )
                        .at(loc()),
                    ...
                    col(unifier.fresh_type())
                } else if let Some(scheme) = env.lookup(&func.node) {
                    // Pure function — check if bridgeable
                    if !scheme.is_mono() {
                        // Polymorphic — can't bridge without concrete types
                        unifier.push_error(
                            Diagnostic::error(
                                Category::TypeError,
                                format!(
                                    "polymorphic function `{}` cannot be used in column expressions \
                                     — add type annotations to make it monomorphic",
                                    func.node
                                ),
                            )
                            .at(loc()),
                        ...
                        // Monomorphic pure function — synthesize return type
                        match &scheme.ty {
                            Type::Function(ft) => {
                                // Check for aggregate shape: fn(List(T)) -> U
                                // In ColumnExpr, this takes Col(T) and returns Col(Option(U))
                                if ft.params.len() == 1
                                    && let Type::List(elem_ty) = &ft.params[0]
                                    && elem_ty.is_arrow_scalar()
                                    && ft.ret.is_arrow_scalar()
                                {
                                    // Aggregate bridge: unify arg with element type
                                    if arg_inners.len() != 1 {
                                        unifier.push_error(
                                            Diagnostic::error(
                                                Category::TypeError,
                                                format!(
                                                    "aggregate function `{}` expects 1 argument, got {}",
                                                    func.node, arg_inners.len()
                                                ),
                                            )
                                            .at(loc()),
                                        );
                                    ...
                                    return col(Type::Option(Box::new(ft.ret.as_ref().clone())));
                                }

                                // Scalar bridge: check Arrow compatibility
                                let params_ok = ft.params.iter().all(|p| p.is_arrow_scalar());
                                let ret_ok = ft.ret.is_arrow_scalar();

                                if params_ok && ret_ok {
                                    // Unify each argument with the expected param type
                                    for (arg_inner, param_ty) in
                                        arg_inners.iter().zip(ft.params.iter())
                                    {
                                        constrain_type_eq(
                                            unifier,
                                            arg_inner,
                                    ...
                                    // Arity check
                                    if arg_inners.len() != ft.params.len() {
                                        unifier.push_error(
                                            Diagnostic::error(
                                                Category::TypeError,
                                                format!(
                                    ...
                                    col(ft.ret.as_ref().clone())
                                } else {
                                    unifier.push_error(
                                        Diagnostic::error(
                                            Category::TypeError,
                                            format!(
                                                "function `{}` has non-Arrow parameter or return types \
                                                 and cannot be used in column expressions",
                                                func.node
                                            ),
                                        )
                                        .at(loc()),
                            ...
                            _ => {
                                unifier.push_error(
                                    Diagnostic::error(
                                        Category::UndefinedName,
                                        format!(
                                            "function `{}` is not registered for column expressions",
                                            func.node
                                        ),
                                    )
                                    .at(loc()),
                    ...
                    // Pure but not in type env (shouldn't happen normally)
                    unifier.push_error(
                        Diagnostic::error(
                            Category::UndefinedName,
                            format!(
                ...
                // No effects registered — unknown function
                unifier.push_error(
                    Diagnostic::error(
                        Category::UndefinedName,
                        format!(
        ...
        ColExprKind::Cond { arms } => {
            let result_inner = unifier.fresh_type();
            let mut has_catch_all = false;

            for arm in arms {
                if matches!(arm.condition.node, ColExprKind::Wildcard) {
                    has_catch_all = true;
                } else {
                    let condition_col = infer_col_expr(&arm.condition, df_ty, env, unifier, span);
                    let condition_inner = unwrap_col(&condition_col);
                    let (condition_base, _) = col_unwrap_option(&condition_inner);
                    constrain_type_eq(
                        unifier,
                        condition_base,
                        &Type::Bool,
                ...
                let body_col = infer_col_expr(&arm.body, df_ty, env, unifier, span);
                let body_inner = unwrap_col(&body_col);
                constrain_type_eq(
                    unifier,
                    &result_inner,
                    &body_inner,
            ...
            col(unifier.substitution.apply(&result_inner))
        }

        ColExprKind::Wildcard => {
            unifier.push_error(
                Diagnostic::error(
                    Category::Syntax,
        ...
        ColExprKind::List(items) => {
            // Infer element type from items and return List(T).
            let elem_ty = unifier.fresh_type();
            for item in items {
                let item_col = infer_col_expr(item, df_ty, env, unifier, span);
                let item_inner = unwrap_col(&item_col);
                constrain_type_eq(unifier, &elem_ty, &item_inner, &prov(Reason::ColumnExpr));
            }
            // List itself isn't a Column type — it's used as RHS of `in`.
        ...
        ColExprKind::Range {
            ...
            let start_col = infer_col_expr(start, df_ty, env, unifier, span);
            let end_col = infer_col_expr(end, df_ty, env, unifier, span);
            let start_inner = unwrap_col(&start_col);
            let end_inner = unwrap_col(&end_col);
            let (start_base, _) = col_unwrap_option(&start_inner);
            let (end_base, _) = col_unwrap_option(&end_inner);
            constrain_type_eq(unifier, start_base, end_base, &prov(Reason::ColumnExpr));
            Type::Opaque {
                name: "Range".to_string(),
                params: vec![start_base.clone()],
            }
        }
    }
}

/// Type-level join validation: checks that key types match and non-key columns
/// are disjoint.
///
/// Returns the merged DataFrame type on success.
pub fn df_join_check(
    left_row: &RowType,
    right_row: &RowType,
    key_labels: &[Label],
    unifier: &mut Unifier,
    ...
    let prov = Provenance {
        span,
        reason: Reason::DataFrameJoin,
    };

    // Check key types match between left and right
    for key in key_labels {
        let left_key_ty = left_row.fields.iter().find(|(l, _)| l == key);
        let right_key_ty = right_row.fields.iter().find(|(l, _)| l == key);

        match (left_key_ty, right_key_ty) {
            (Some((_, lt)), Some((_, rt))) => {
                constrain_type_eq(unifier, lt, rt, &prov);
            }
            (None, _) => {
                return Err(Diagnostic::error(
                    Category::MissingField,
                    format!("join key `{key}` not found in left DataFrame"),
                ));
            }
            (_, None) => {
                return Err(Diagnostic::error(
                    Category::MissingField,
                    format!("join key `{key}` not found in right DataFrame"),
                ));
            }
        }
    }

    // Check non-key columns are disjoint
    let left_non_keys: Vec<_> = left_row
        .fields
        .iter()
        .filter(|(l, _)| !key_labels.contains(l))
        .collect();
    let right_non_keys: Vec<_> = right_row
        .fields
        .iter()
        .filter(|(l, _)| !key_labels.contains(l))
        .collect();

    for (left_label, _) in &left_non_keys {
        for (right_label, _) in &right_non_keys {
            if left_label == right_label {
                return Err(Diagnostic::error(
                    Category::DuplicateField,
                    format!(
                        "join would produce duplicate column `{left_label}` — rename or select before joining"
                    ),
                ));
            }
        }
    ...
    // Build merged row: keys + left non-keys + right non-keys
    let mut merged_fields: Vec<(Label, Type)> = Vec::new();
    for key in key_labels {
        if let Some((_, ty)) = left_row.fields.iter().find(|(l, _)| l == key) {
            merged_fields.push((key.clone(), ty.clone()));
        }
    }
    for (l, ty) in &left_non_keys {
        merged_fields.push(((*l).clone(), ty.clone()));
    }
    for (l, ty) in &right_non_keys {
        merged_fields.push(((*l).clone(), ty.clone()));
    }

    let merged_row = RowType::closed(merged_fields);
    Ok(Type::DataFrame(Box::new(Type::AnonRecord(merged_row))))
}
    ...
    match ann {
        ...
        TypeAnnotation::Applied(name, args) => {
            ...
            match (name.as_str(), args.as_slice()) {
                ...
                ("DataFrame", [schema]) => Some(Type::DataFrame(Box::new(resolve_annotation(
                    schema, records, sum_types,
                )?))),
                ("Column", [inner]) => Some(Type::Column(Box::new(resolve_annotation(
                    inner, records, sum_types,
                )?))),
                ...
                ("GroupedFrame", [schema]) => Some(Type::GroupedFrame {
                    row: Box::new(resolve_annotation(schema, records, sum_types)?),
                    keys: vec![],
                }),
                ("Tagged", [inner]) => Some(Type::Tagged {
...
/// Resolve a type annotation with unifier support for bare `DataFrame`.
///
/// Decision 11: bare `DataFrame` (no schema parameter) resolves to
/// `DataFrame(#{ | r })` — a DataFrame with a fresh open row variable.
/// The compiler infers column requirements from body usage.
///
/// Falls back to `resolve_annotation` for all other annotations.
fn resolve_annotation_or_bare_df(
    ...
    match ann {
        TypeAnnotation::Named(name) => {
            if name == "DataFrame" {
                let row_var = unifier.fresh_row_var();
                return Some(Type::DataFrame(Box::new(Type::AnonRecord(
                    kea_types::RowType {
                        fields: vec![],
                        rest: Some(row_var),
                    },
        ...
        TypeAnnotation::Applied(name, args) => {
            ...
            match (name.as_str(), args.as_slice()) {
                ...
                ("DataFrame", [schema]) => Some(Type::DataFrame(Box::new(
                    resolve_annotation_or_bare_df(schema, records, sum_types, unifier)?,
                ))),
                ("Column", [inner]) => Some(Type::Column(Box::new(resolve_annotation_or_bare_df(
                    inner, records, sum_types, unifier,
                )?))),
                ...
                ("GroupedFrame", [schema]) => Some(Type::GroupedFrame {
                    row: Box::new(resolve_annotation_or_bare_df(
                        schema, records, sum_types, unifier,
                    )?),
                    keys: vec![],
                }),
                ("Tagged", [inner]) => Some(Type::Tagged {
...
#[derive(Clone)]
struct SqlTableScope {
    /// Names usable as qualifiers in SQL (`FROM t AS x` -> `x` and maybe `t`).
    names: Vec<String>,
    row: RowType,
}

fn row_from_dataframe_type(ty: &Type) -> Option<RowType> {
    match ty {
        Type::DataFrame(inner) => match inner.as_ref() {
            Type::AnonRecord(row) => Some(row.clone()),
            Type::Record(rt) => Some(rt.row.clone()),
            _ => None,
        },
        _ => None,
...
fn table_scopes_from_from_clause(
    from: &[TableWithJoins],
    env: &TypeEnv,
    unifier: &Unifier,
    sql_bindings: &BTreeMap<String, RowType>,
) -> Vec<SqlTableScope> {
    let mut scopes = Vec::new();
    for twj in from {
        collect_table_factor_scope(&twj.relation, env, unifier, sql_bindings, &mut scopes);
        for join in &twj.joins {
            collect_table_factor_scope(&join.relation, env, unifier, sql_bindings, &mut scopes);
        }
    }
    scopes
}
...
fn collect_table_factor_scope(
    ...
    unifier: &Unifier,
    sql_bindings: &BTreeMap<String, RowType>,
    out: &mut Vec<SqlTableScope>,
) {
    match factor {
        TableFactor::Table { name, alias, .. } => {
            if let Some(base_name) = object_name_last(name) {
                let row = sql_bindings.get(&base_name).cloned().or_else(|| {
                    env.lookup(&base_name).and_then(|scheme| {
                        row_from_dataframe_type(&unifier.substitution.apply(&scheme.ty))
                    })
                });
                if let Some(row) = row {
                    ...
                    out.push(SqlTableScope { names, row });
                }
            }
        }
        TableFactor::Derived {
            ...
            if let Some(row) = infer_sql_query_row(subquery, env, unifier, sql_bindings)
                .ok()
                .flatten()
            {
                let names = alias
...
fn sql_expr_type(expr: &SqlExpr, scopes: &[SqlTableScope]) -> Result<Type, String> {
    match expr {
        SqlExpr::Identifier(id) => resolve_column_type(None, &id.value, scopes),
        SqlExpr::CompoundIdentifier(parts) if parts.len() == 2 => {
            resolve_column_type(Some(&parts[0].value), &parts[1].value, scopes)
        }
        _ => Ok(Type::Dynamic),
    }
}

fn sql_expr_type_with_fallback(
    expr: &SqlExpr,
    scopes: &[SqlTableScope],
    allow_unresolved_columns: bool,
) -> Result<Type, String> {
    match sql_expr_type(expr, scopes) {
        Ok(ty) => Ok(ty),
        Err(_) if allow_unresolved_columns => Ok(Type::Dynamic),
        Err(e) => Err(e),
    }
}
...
fn resolve_column_type(
    ...
    for scope in scopes {
        if let Some(q) = qualifier
            && !scope.names.iter().any(|name| name == q)
        {
            continue;
        }
        if let Some((_, ty)) = scope.row.fields.iter().find(|(l, _)| l.as_str() == col) {
    ...
    match matches.len() {
        ...
        0 => {
            if let Some(q) = qualifier {
                Err(format!("unknown column `{q}.{col}`"))
            } else {
        ...
        _ => {
            if let Some(q) = qualifier {
                Err(format!("ambiguous column `{q}.{col}`"))
            } else {
                Err(format!("ambiguous column `{col}`"))
            }
        }
    }
}

fn infer_sql_select_row(
    body: &str,
    env: &TypeEnv,
    unifier: &Unifier,
) -> Result<Option<RowType>, String> {
    let dialect = GenericDialect {};
    let statements = SqlParser::parse_sql(&dialect, body).map_err(|e| e.to_string())?;
    ...
    infer_sql_query_row(query, env, unifier, &BTreeMap::new())
}

fn infer_sql_query_row(
    query: &sqlparser::ast::Query,
    env: &TypeEnv,
    unifier: &Unifier,
    sql_bindings: &BTreeMap<String, RowType>,
) -> Result<Option<RowType>, String> {
    let mut local_bindings = sql_bindings.clone();

    if let Some(with) = &query.with {
        for cte in &with.cte_tables {
            let Some(mut row) = infer_sql_query_row(&cte.query, env, unifier, &local_bindings)?
            else {
                continue;
            };
            if !cte.alias.columns.is_empty() {
                if cte.alias.columns.len() != row.fields.len() {
                    return Err(format!(
                        "CTE `{}` alias column count ({}) does not match query output columns ({})",
                ...
                let fields = row
                    ...
                    .map(|((_, ty), col)| (Label::new(col.name.value.clone()), ty))
                    .collect();
                row = RowType::closed(fields);
            }
            local_bindings.insert(cte.alias.name.value.clone(), row);
        }
    }

    infer_sql_setexpr_row(query.body.as_ref(), env, unifier, &local_bindings)
}

fn infer_sql_setexpr_row(
    set_expr: &SetExpr,
    env: &TypeEnv,
    unifier: &Unifier,
    sql_bindings: &BTreeMap<String, RowType>,
    ...
    match set_expr {
        SetExpr::Select(select) => infer_sql_select_ast_row(select, env, unifier, sql_bindings),
        SetExpr::Query(query) => infer_sql_query_row(query, env, unifier, sql_bindings),
        SetExpr::SetOperation { left, right, .. } => {
            let left_row = infer_sql_setexpr_row(left, env, unifier, sql_bindings)?;
            let right_row = infer_sql_setexpr_row(right, env, unifier, sql_bindings)?;
            match (left_row, right_row) {
                (Some(l), Some(r)) => {
                    ...
                    let fields = l
                        ...
                        .zip(&r.fields)
                        .map(|((label, lt), (_, rt))| {
                            let ty = if lt == rt { lt.clone() } else { Type::Dynamic };
                            (label.clone(), ty)
                        })
                        .collect();
                    Ok(Some(RowType::closed(fields)))
                }
                _ => Ok(None),
            }
        }
...
fn infer_sql_select_ast_row(
    select: &sqlparser::ast::Select,
    env: &TypeEnv,
    unifier: &Unifier,
    sql_bindings: &BTreeMap<String, RowType>,
) -> Result<Option<RowType>, String> {
    let relation_count = count_table_relations(&select.from);
    let scopes = table_scopes_from_from_clause(&select.from, env, unifier, sql_bindings);
    let allow_unresolved_columns = relation_count > scopes.len();
    ...
    for item in &select.projection {
        match item {
            SelectItem::Wildcard(_) => {
                for scope in &scopes {
                    fields.extend(scope.row.fields.iter().cloned());
                }
            }
            SelectItem::QualifiedWildcard(kind, _) => {
                let SelectItemQualifiedWildcardKind::ObjectName(obj_name) = kind else {
                    continue;
                };
                let Some(qname) = object_name_last(obj_name) else {
                    continue;
                };
                for scope in &scopes {
                    if scope.names.iter().any(|n| n == &qname) {
                        fields.extend(scope.row.fields.iter().cloned());
                    }
                }
            }
            SelectItem::ExprWithAlias { expr, alias } => {
                let ty = sql_expr_type_with_fallback(expr, &scopes, allow_unresolved_columns)?;
                fields.push((Label::new(alias.value.clone()), ty));
            }
...
fn count_table_relations(from: &[TableWithJoins]) -> usize {
...
fn resolve_bound_call_args(
    ...
    if io_reader_name(func).is_some() {
        let fallback = args
            .iter()
            .map(|arg| BoundArg::Provided(arg.value.clone()))
            .collect::<Vec<_>>();
        if args.len() == 1 {
            if args[0].label.is_some() {
                unifier.push_error(
                    Diagnostic::error(
                        Category::ArityMismatch,
                        "path argument must be positional in `read_csv`/`read_parquet` calls",
                    )
        ...
        if args.len() == 2 {
            if args[0].label.is_some() {
                unifier.push_error(
                    Diagnostic::error(
                        Category::ArityMismatch,
                        "first argument (path) must be positional in `read_csv`/`read_parquet` calls",
                    )
                    .at(span_to_loc(args[0].value.span)),
                );
            }
            if let Some(label) = &args[1].label
                && label.node != "schema"
            {
                unifier.push_error(
                    Diagnostic::error(
                        Category::ArityMismatch,
                        format!("unknown argument label `{}`; expected `schema`", label.node),
                    )
        ...
        unifier.push_error(
            Diagnostic::error(
                Category::ArityMismatch,
                format!(
                    "readers `read_csv`/`read_parquet` take 1 or 2 arguments, got {}",
                    args.len()
                ),
            )
    ...
    match &expr.node {
        ...
        ExprKind::Call { func, args } => {
            ...
            if let Some(expect_type_ty) = infer_expect_type_call(
                ...
                return expect_type_ty;
            }
            if let Some(io_schema_ty) = infer_io_reader_schema_call(
                func,
                &bound_args,
                call_signature.as_ref(),
                env,
        ...
        ExprKind::Frame { columns } => {
            if columns.is_empty() {
                unifier.push_error(
                    Diagnostic::error(
                        Category::TypeMismatch,
                        "frame literal must have at least one column",
                    )
                    .at(span_to_loc(expr.span)),
                );
                return Type::DataFrame(Box::new(Type::Dynamic));
            }
            let mut field_types: Vec<(Label, Type)> = Vec::new();
            let mut expected_len: Option<usize> = None;
            let mut seen_columns: std::collections::BTreeSet<String> =
                std::collections::BTreeSet::new();
            for (col_name, values) in columns {
                // Check for duplicate column names
                if !seen_columns.insert(col_name.node.clone()) {
                    unifier.push_error(
                        Diagnostic::error(
                            Category::DuplicateField,
                            format!("duplicate column `{}` in frame literal", col_name.node),
                        )
                        .at(span_to_loc(col_name.span)),
                    );
                    continue;
                }
                if values.is_empty() {
                    unifier.push_error(
                        Diagnostic::error(
                            Category::TypeMismatch,
                            format!("column `{}` must have at least one value", col_name.node),
                        )
                        .at(span_to_loc(col_name.span)),
                    );
                    continue;
                    ...
                    Some(n) if n != values.len() => {
                        unifier.push_error(
                            Diagnostic::error(
                                Category::TypeMismatch,
                                format!(
                                    "column `{}` has {} values, but expected {} (matching first column)",
                                    col_name.node,
                                    values.len(),
                                    n,
                                ),
                ...
                // Infer element type: all values must unify.
                // Special case: if any value is `None`, auto-promote the column
                // to `Option(T)` where T is inferred from the non-None values.
                // This is a frame-literal-only rule — data specification, not computation.
                let col_label = Label::new(col_name.node.clone());
                let has_none = values.iter().any(|v| matches!(v.node, ExprKind::None));
                if has_none {
                    // Separate None from non-None values
                    let non_none: Vec<_> = values
                        .iter()
                        .filter(|v| !matches!(v.node, ExprKind::None))
                        .collect();
                    if non_none.is_empty() {
                        // All values are None — can't infer the inner type
                        unifier.push_error(
                            Diagnostic::error(
                                Category::TypeMismatch,
                                format!(
                                    "column `{}` has only None values — cannot infer column type",
                                    col_name.node,
                                ),
                            )
                            .at(span_to_loc(col_name.span))
                            .with_help("add at least one non-None value, or use a type annotation"),
                        );
                        field_types.push((col_label, Type::Option(Box::new(Type::Dynamic))));
                    } else {
                        // Infer T from non-None values, then column type is Option(T)
                        let elem_ty =
                            infer_expr_bidir(non_none[0], env, unifier, records, traits, sum_types);
                        for val in &non_none[1..] {
                            let val_ty =
                                infer_expr_bidir(val, env, unifier, records, traits, sum_types);
                            constrain_type_eq(
                                unifier,
                                &elem_ty,
                                &val_ty,
                                &prov(Reason::FrameColumnValue {
                                    column: col_label.clone(),
                                }),
                            );
                        }
                        // Infer None values so their fresh vars get unified with elem_ty
                        for val in values.iter().filter(|v| matches!(v.node, ExprKind::None)) {
                            let none_ty =
                                infer_expr_bidir(val, env, unifier, records, traits, sum_types);
                            // none_ty is Option(?a) — unify ?a with elem_ty via Option
                            constrain_type_eq(
                                unifier,
                        ...
                        field_types.push((col_label, Type::Option(Box::new(elem_ty))));
                    }
                } else {
                    // No None values — standard inference, column is non-nullable
                    let first_ty =
                        infer_expr_bidir(&values[0], env, unifier, records, traits, sum_types);
                    for val in &values[1..] {
                        let val_ty =
                            infer_expr_bidir(val, env, unifier, records, traits, sum_types);
                        constrain_type_eq(
                    ...
                    field_types.push((col_label, first_ty));
                }
            }
            let row = RowType::closed(field_types);
        ...
        // -- Embedded blocks (sql/html/markdown) --
        ExprKind::EmbeddedBlock {
            kind,
            parts,
            atom_fields,
            ...
            kea_ast::BlockKind::Sql => {
                // Type-check each interpolated expression.
                for part in parts {
                    if let kea_ast::StringInterpPart::Expr(inner) = part {
                        infer_expr_bidir(inner, env, unifier, records, traits, sum_types);
                    }
                }
                        ...
                        match key.node.as_str() {
                            "source" => {
                                ...
                                constrain_type_eq(
                                    unifier,
                                    &value_ty,
                                    &connection_opaque_type(),
                                    &prov(Reason::LetAnnotation),
                                );
                            }
                            "dialect" => constrain_type_eq(
                                unifier,
                                &value_ty,
                                &Type::Atom,
                                &prov(Reason::LetAnnotation),
                            ),
                            "schema" => constrain_type_eq(
                                unifier,
                                &value_ty,
                                &Type::String,
                                &prov(Reason::LetAnnotation),
                            ),
                            "read_only" => constrain_type_eq(
                                unifier,
                                &value_ty,
                                &Type::Bool,
                                &prov(Reason::LetAnnotation),
                            ),
                            "timeout" => constrain_type_eq(
                                unifier,
                                &value_ty,
                                &Type::Int,
                                &prov(Reason::LetAnnotation),
                            ),
                            other => unifier.push_error(
                                Diagnostic::warning(
                                    Category::TypeError,
                                    format!(
                                        "unknown sql config field `{other}` (expected source, dialect, timeout, schema, read_only)"
                                    ),
                                )
                                .at(span_to_loc(key.span)),
                            ),
                        }
                    }
                }
                if has_source {
                    // Remote schema is runtime-dependent; keep SQL with explicit source conservative.
                    return Type::DataFrame(Box::new(Type::Dynamic));
                }
                // Reconstruct SQL body for schema inference.
                let (sql_body, _) = kea_ast::reconstruct_sql_body(parts);
                match infer_sql_select_row(&sql_body, env, unifier) {
                    Ok(Some(row)) => Type::DataFrame(Box::new(Type::AnonRecord(row))),
                    Ok(None) => Type::DataFrame(Box::new(Type::Dynamic)),
                    Err(msg) => {
                        unifier.push_error(
                            Diagnostic::error(
                                Category::TypeError,
                                format!("invalid sql block: {msg}"),
                            )
                            .at(SourceLocation {
                                file_id: expr.span.file.0,
                                start: expr.span.start,
                                end: expr.span.end,
                            }),
                        );
                        Type::DataFrame(Box::new(Type::Dynamic))
                    }
                }
            }
            kea_ast::BlockKind::Html | kea_ast::BlockKind::Markdown => {
                // Row-polymorphic function parameter synthesized from {:field} refs.
                let mut seen = BTreeSet::<String>::new();
                let mut fields = Vec::<(Label, Type)>::new();
                for atom in atom_fields {
                    if seen.insert(atom.node.clone()) {
                        fields.push((Label::new(atom.node.clone()), unifier.fresh_type()));
                    }
                }
                let row_var = unifier.fresh_row_var();
                let arg_row = RowType::open(fields, row_var);
                let row_ty = Type::AnonRecord(arg_row.clone());

                env.push_scope();
                env.bind("__row".to_string(), TypeScheme::mono(row_ty.clone()));
                for part in parts {
                    if let kea_ast::StringInterpPart::Expr(inner) = part {
                        infer_expr_bidir(inner, env, unifier, records, traits, sum_types);
                    }
                ...
                let ret_ty = match kind {
                    kea_ast::BlockKind::Html => Type::Html,
                    kea_ast::BlockKind::Markdown => Type::Markdown,
                    kea_ast::BlockKind::Sql => unreachable!(),
                };
        ...
        ExprKind::DfVerb { .. } => {
            // DfVerb type inference is handled in Step 4 (infer_df_verb).
            // For now, return a type error — DfVerb must appear as RHS of |>.
            unifier.push_error(
                Diagnostic::error(
                    Category::TypeError,
                    "DataFrame verb cannot be used as a standalone expression (use with |>)",
                )
                .at(SourceLocation {
                    file_id: expr.span.file.0,
                    start: expr.span.start,
    ...
    match ann {
        ...
        TypeAnnotation::Applied(name, args) => {
            ...
            match (name.as_str(), args.as_slice()) {
                ...
                ("DataFrame", [inner]) => Some(Type::DataFrame(Box::new(
                    resolve_annotation_with_self_and_assoc(inner, records, self_type, assoc_types)?,
                ))),
                ("Column", [inner]) => Some(Type::Column(Box::new(
                    resolve_annotation_with_self_and_assoc(inner, records, self_type, assoc_types)?,
                ))),
                ...
                ("GroupedFrame", [schema]) => Some(Type::GroupedFrame {
                    row: Box::new(resolve_annotation_with_self_and_assoc(
                        schema, records, self_type, assoc_types,
                    )?),
                    keys: vec![],
                }),
    ...
    match ann {
        ...
        TypeAnnotation::Applied(name, args) => {
            ...
            match (name.as_str(), args.as_slice()) {
                ...
                ("GroupedFrame", [schema]) => Some(Type::GroupedFrame {
                    row: Box::new(resolve_annotation_with_self_assoc_and_params(
                        schema,
                        records,
                        self_type,
                        assoc_types,
