...
/--
Capability-extended shape-parameterized typed-handle capstone:
adds normalized and closed-aware capability bundle consequences under explicit
capability-origin premises.
-/
theorem handler_typed_handle_shape_capstone_with_capability
    {tenv : TermEnv}
    {body : HandlerExpr}
    {handler : HandleContract}
    {clause : HandlerClauseSem}
  ...
  have h_base :
      ...
      (TailResumptiveClassification.classifyClause clause.contract ≠
        TailResumptiveClassification.TailResumptiveClass.invalid) ∧
      TailResumptiveClassification.TailResumptiveBundle clause.contract :=
    handler_typed_handle_shape_capstone h_typed h_shape
  have h_cap :
      TailCapabilityComposition.TailCapabilityBundle clause.contract capability :=
    handler_clause_tail_capability_bundle_of_handlerHasType h_typed h_expr h_ne
  have h_cap_closed :
    ...
    handler_clause_tail_capability_closedAware_bundle_of_handlerHasType
      h_typed h_expr h_ne
  exact ⟨h_base.1, h_base.2.1, h_base.2.2.1, h_base.2.2.2.1, h_base.2.2.2.2, h_cap, h_cap_closed⟩

/--
Core-body specialization of `handler_typed_handle_shape_capstone`.
-/
theorem handler_typed_core_body_capstone
    {tenv : TermEnv}
    {e : CoreExpr}
    {handler : HandleContract}
    {clause : HandlerClauseSem}
    ...
    (h_typed : HandlerHasType tenv (.handle (.core e) handler clause) ty) :
    (∃ e',
      HandlerStep (.handle (.core e) handler clause) e' ∧
      HandlerHasType tenv e' ty) ∧
    HandleClauseContract.wellTypedSlice clause.contract ∧
    resume_at_most_once clause.contract.resumeUse ∧
    (TailResumptiveClassification.classifyClause clause.contract ≠
...
/--
Capability-extended core-body specialization of
`handler_typed_handle_shape_capstone_with_capability`.
-/
theorem handler_typed_core_body_capstone_with_capability
    {tenv : TermEnv}
    {e : CoreExpr}
    {handler : HandleContract}
    {clause : HandlerClauseSem}
...
consequences on the same boundary route.
-/
theorem handler_typed_redex_eval_and_contract_capstone
    {tenv : TermEnv}
    {venv : ValueEnv}
    {handler : HandleContract}
    {clause : HandlerClauseSem}
...
contracts and both normalized/closed-aware capability bundle outputs.
-/
theorem handler_typed_redex_eval_and_capability_contract_capstone
    {tenv : TermEnv}
    {venv : ValueEnv}
    {handler : HandleContract}
    {clause : HandlerClauseSem}
...
/--
Strengthened generic evaluator bridge for supported-shape typed handles:
in addition to step/eval/value-typing, the stepped-to core target is also
typed at `ty`.
-/
theorem handler_typed_handle_shape_eval_sound_typed_target
    {tenv : TermEnv}
    {venv : ValueEnv}
    {body : HandlerExpr}
    {handler : HandleContract}
    ...
    ∃ target v,
      HandlerStep (.handle body handler clause) (.core target)
      ∧ HasType tenv target ty
      ∧ eval venv target = some v
      ∧ ValueHasType v ty := by
  rcases handler_typed_handle_shape_eval_sound h_env h_typed h_shape h_frag with
    ⟨target, v, h_step, h_eval_eq, h_v_ty⟩
  have h_target_typed : HasType tenv target ty :=
    handlerHasType_core_inv (handler_step_preservation h_typed h_step)
  exact ⟨target, v, h_step, h_target_typed, h_eval_eq, h_v_ty⟩

/--
Strengthened generic shape-based evaluator+contract capstone:
adds a typed core-target witness to the existing evaluator+contract outputs.
-/
theorem handler_typed_handle_shape_eval_and_contract_typed_target_capstone
    {tenv : TermEnv}
    {venv : ValueEnv}
    {body : HandlerExpr}
    {handler : HandleContract}
...
/--
Capability-extended strengthened generic evaluator+contract capstone:
adds a typed core-target witness to the existing capability contract outputs.
-/
theorem handler_typed_handle_shape_eval_and_capability_contract_typed_target_capstone
    {tenv : TermEnv}
    {venv : ValueEnv}
    {body : HandlerExpr}
    {handler : HandleContract}
...
/--
Perform-redex specialization: direct core-soundness consequence plus clause and
capability contract consequences.
-/
theorem handler_typed_redex_core_soundness_and_capability_contract_capstone
    {tenv : TermEnv}
    {venv : ValueEnv}
    {handler : HandleContract}
    {clause : HandlerClauseSem}
  ...
  have h_frag_shape :
    ...
    | handle_perform_tail _ _ _ _ _ _ _ _ =>
        simpa [bindTailResumptive] using h_frag
  rcases handler_typed_handle_shape_core_soundness_and_capability_contract_capstone
      h_env h_typed h_shape h_frag_shape h_expr h_ne with
    ⟨h_core_exists, h_wellTyped, h_linear, h_not_invalid, h_tail_bundle, h_cap, h_cap_closed⟩
  rcases h_core_exists with ⟨target, h_step, h_cons⟩
  cases h_step with
  | handle_perform_tail _ _ _ _ _ _ _ _ =>
      exact ⟨
...
/--
Core-body specialization: direct core-soundness consequence plus clause and
capability contract consequences.
-/
theorem handler_typed_core_body_core_soundness_and_capability_contract_capstone
    {tenv : TermEnv}
    {venv : ValueEnv}
    {e : CoreExpr}
    {handler : HandleContract}
